...
int rng_rand(RNG_state *mystate) {
  int r;
  uint32 b = (mystate[16] << 24) | (mystate[17] << 16) | (mystate[18] << 8) |
             (mystate[19] << 0);
  b = b & POS_MASK;
...
int rng_nextrand(RNG_state *mystate) {
  ...
  b = (mystate[16] << 24) | (mystate[17] << 16) | (mystate[18] << 8) |
      (mystate[19] << 0);
  b = b & POS_MASK;
...
VOID_RETURN sha1_hash(const unsigned char data[], unsigned long len,
                      sha1_ctx ctx[1]) {
...
VOID_RETURN sha1(unsigned char hval[], const unsigned char data[],
                 unsigned long len) {
  sha1_ctx cx[1];
