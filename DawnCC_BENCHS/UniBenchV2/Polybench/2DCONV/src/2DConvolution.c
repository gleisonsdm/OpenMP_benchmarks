...
void conv2D(DATA_TYPE *A, DATA_TYPE *B) {
  ...
  #pragma omp target data map(to: A[0:67108864]) map(tofrom: B[8193:67092478]) if(!RST_AI1)
...
void init(DATA_TYPE *A) {
  ...
  #pragma omp target data map(tofrom: A[0:67108864])
  #pragma omp target
  for (i = 0; i < NI; ++i) {
    for (j = 0; j < NJ; ++j) {
...
void compareResults(DATA_TYPE *B, DATA_TYPE *B_GPU) {
  ...
  // Compare B and B_GPU
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (B + 8193) > (void*) (B_GPU + 67092478))
  || ((void*) (B_GPU + 8193) > (void*) (B + 67092478)));
  #pragma omp target data map(to: B[8193:67092478],B_GPU[8193:67092478])  if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 1; i < (NI - 1); i++) {
    for (j = 1; j < (NJ - 1); j++) {
