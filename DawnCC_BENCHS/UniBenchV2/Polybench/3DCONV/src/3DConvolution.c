...
void conv3D(DATA_TYPE *A, DATA_TYPE *B) {
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (A + 0) > (void*) (B + 133692414))
  || ((void*) (B + 262657) > (void*) (A + 134217728)));
  #pragma omp target data map(to: A[0:134217728]) map(tofrom: B[262657:133692414]) if(!RST_AI1)
...
void init(DATA_TYPE *A) {
  ...
  #pragma omp target data map(tofrom: A[0:134217728])
  #pragma omp target
  for (i = 0; i < NI; ++i) {
    for (j = 0; j < NJ; ++j) {
      for (k = 0; k < NK; ++k) {
...
void compareResults(DATA_TYPE *B, DATA_TYPE *B_GPU) {
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (B + 262657) > (void*) (B_GPU + 133692414))
  || ((void*) (B_GPU + 262657) > (void*) (B + 133692414)));
  #pragma omp target data map(to: B[262657:133692414],B_GPU[262657:133692414])  if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 1; i < NI - 1; ++i) {
    for (j = 1; j < NJ - 1; ++j) {
      for (k = 1; k < NK - 1; ++k) {
