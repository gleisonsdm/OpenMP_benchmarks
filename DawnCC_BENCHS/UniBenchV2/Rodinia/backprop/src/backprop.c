...
int m;
{
  ...
  AI1[3] = (AI1[2] > 0);
  AI1[4] = (AI1[3] ? AI1[2] : 0);
  #pragma omp target data map(tofrom: w[0:AI1[4]])
  #pragma omp target
  for (i = 0; i <= m; i++) {
...
void compareResults(float *l2, float *l2_gpu, int n2) {
  ...
  #pragma omp target data map(to: l2[AI1[11]:AI1[12]],l2_gpu[AI1[11]:AI1[12]])  if(!RST_AI1)
...
#pragma omp parallel for
    ...
    #pragma omp target data map(to: l1[0:AI1[4]]) map(tofrom: conn_gpu[AI1[19]:AI1[20]]) if(!RST_AI1)
    #pragma omp target if(!RST_AI1)
    for (j = 1; j <= n2; j++) {
...
int nj;
{
  ...
  #pragma omp target data map(to: output[AI1[11]:AI1[12]],target[AI1[11]:AI1[12]]) map(tofrom: delta[AI1[11]:AI1[12]]) if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (j = 1; j <= nj; j++) {
    o = output[j];
    t = target[j];
...
#pragma omp parallel for collapse(1)
    ...
    #pragma omp target data map(to: delta[AI1[11]:AI1[12]],ly[0:AI1[17]]) map(tofrom: oldw_gpu[AI1[31]:AI1[32]],w_gpu[AI1[31]:AI1[32]]) if(!RST_AI1)
    #pragma omp target if(!RST_AI1)
    for (j = 1; j <= ndelta; j++) {
      for (k = 0; k <= nly; k++) {
        new_dw =
