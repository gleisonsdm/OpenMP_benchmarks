...
#pragma omp target map(to : power[0 : row *col], temp[0 : row *col])           \
                           map(tofrom : result[0 : row *col])
  {
...
#pragma omp parallel for shared(result, temp) private(r, c) schedule(static)
  ...
  AI1[8] = (AI1[7] ? AI1[6] : 0);
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (result + 0) > (void*) (temp + AI1[8]))
  || ((void*) (temp + 0) > (void*) (result + AI1[8])));
  #pragma omp target data map(to: result[0:AI1[8]]) map(tofrom: temp[0:AI1[8]]) if(!RST_AI1)
...
void compareResults(double *result_cpu, double *result_gpu, int grid_rows,
                    int grid_cols) {
  int i, fail;
  fail = 0;

  long long int AI1[7];
  AI1[0] = grid_rows * grid_cols;
  AI1[1] = AI1[0] + -1;
  AI1[2] = 8 * AI1[1];
  AI1[3] = AI1[2] + 8;
  AI1[4] = AI1[3] / 8;
  ...
  AI1[6] = (AI1[5] ? AI1[4] : 0);
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (result_cpu + 0) > (void*) (result_gpu + AI1[6]))
  || ((void*) (result_gpu + 0) > (void*) (result_cpu + AI1[6])));
  #pragma omp target data map(to: result_cpu[0:AI1[6]],result_gpu[0:AI1[6]])  if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < grid_rows * grid_cols; i++) {
