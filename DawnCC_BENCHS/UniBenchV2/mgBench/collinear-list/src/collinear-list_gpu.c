...
int colinear_list_points_GPU() {
  ...
  #pragma omp target data map(tofrom: parallel_lines[0:9999])
  #pragma omp target
  for (i = 0; i < p; i++) {
    parallel_lines[i] = 0;
  }
...
#pragma omp target map(to : points[0 : SIZE])                                  \
    map(tofrom : parallel_lines[0 : p])
  {
#pragma omp parallel for collapse(3)
    #pragma omp target data map(tofrom: parallel_lines[0:10000])
    #pragma omp target
    for (i = 0; i < SIZE; i++) {
      for (j = 0; j < SIZE; j++) {
        for (k = 0; k < SIZE; k++) {
