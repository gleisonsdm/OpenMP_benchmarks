    ...
    *capital = (struct Village *)malloc(sizeof(struct Village));
    ...
    for (i = sim_cities; i > 0; i--) {
      allocate_village(&current, *capital, inext, level - 1,
                       (vid * (int32_t)sim_cities) + (int32_t)i);
      inext = current;
    }
...
#pragma omp task untied if ((sim_level - village->level) < bots_cutoff_value)
  ...
  /* Uses lists v->hosp->assess, v->hosp->inside, v->population and
   * (v->back->hosp->realloc) !!! */
...
void read_input_data(char *filename) {
  ...
  res = fscanf(fin, "%d %d %d %d %d %d %ld %f %f %f %d %d %d %d %d %d %d %d %f",
               &sim_level, &sim_cities, &sim_population_ratio, &sim_time,
               &sim_assess_time, &sim_convalescence_time, &sim_seed,
               &sim_get_sick_p, &sim_convalescence_p, &sim_realloc_p,
               &res_population, &res_hospitals, &res_personnel, &res_checkin,
               &res_village, &res_waiting, &res_assess, &res_inside,
               &res_avg_stay);
  if (res == EOF) {
