...
void get_info(char *argv[], int *typep, char *classp) {
  ...
  *classp = *argv[2];
  ...
  if (!strcmp(argv[1], "sp") || !strcmp(argv[1], "SP"))
    ...
    *typep = BT;
  else if (!strcmp(argv[1], "ft") || !strcmp(argv[1], "FT"))
    *typep = FT;
  else if (!strcmp(argv[1], "lu") || !strcmp(argv[1], "LU"))
    *typep = LU;
  else if (!strcmp(argv[1], "mg") || !strcmp(argv[1], "MG"))
    *typep = MG;
  else if (!strcmp(argv[1], "is") || !strcmp(argv[1], "IS"))
    *typep = IS;
  else if (!strcmp(argv[1], "ep") || !strcmp(argv[1], "EP"))
    *typep = EP;
  else if (!strcmp(argv[1], "cg") || !strcmp(argv[1], "CG"))
    *typep = CG;
  else {
...
void check_info(int type, char class) {
  ...
  /* check class */
  if (class != 'S' && class != 'A' && class != 'B' && class != 'R' &&
      class != 'W' && class != 'C') {
...
void read_info(int type, char *classp) {
  ...
  case CG:
    ...
    if (nread != 1) {
      printf("setparams: Error parsing config file %s. Ignoring previous "
             "settings\n",
             FILENAME);
      goto abort;
    }
  ...
  default:
    ...
    printf("setparams: (Internal Error) Benchmark type %d unknown to this "
           "program\n",
           type);
    exit(1);
  }
...
void write_cg_info(FILE *fp, char class) {
  ...
  char *shiftS = "10.0", *shiftW = "12.0", *shiftA = "20.0", *shiftB = "60.0",
       *shiftC = "110.0";
