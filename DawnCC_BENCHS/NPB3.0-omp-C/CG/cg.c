...
static void sparse(double a[], int colidx[], int rowstr[], int n, int arow[],
...
#pragma omp for nowait
  ...
  for (it = 1; it <= 1; it++) {
    ...
    norm_temp11 = 0.0;
    norm_temp12 = 0.0;
#pragma omp parallel for default(shared) private(j) reduction(+ : norm_temp11, \
                                                              norm_temp12)
    for (j = 1; j <= lastcol - firstcol + 1; j++) {
...
#pragma omp master
  ...
  c_print_results("CG", class, NA, 0, 0, NITER, nthreads, t, mflops,
...
/*--------------------------------------------------------------------
c-------------------------------------------------------------------*/
static void conj_grad(int colidx[], /* colidx[1:nzz] */
...
#pragma omp for reduction(+ : rho)
  ...
  for (cgit = 1; cgit <= cgitmax; cgit++) {
    ...
    d = 0.0;
    rho = 0.0;
#pragma omp parallel default(shared) private(j, k, sum, alpha,                 \
                                             beta) shared(d, rho0, rho)
    {
...
c       v, aelt        r*8
c---------------------------------------------------------------------*/
static void makea(int n, int nz, double a[], /* a[1:nz] */
...
c---------------------------------------------------*/
static void sparse(double a[],                            /* a[1:*] */
                   int colidx[],                          /* colidx[1:*] */
                   int rowstr[],                          /* rowstr[1:*] */
...
---------------------------------------------------------------------*/
static void sprnvc(int n, int nz, double v[], /* v[1:*] */
                   int iv[],                  /* iv[1:*] */
                   int nzloc[],               /* nzloc[1:n] */
                   int mark[])                /* mark[1:n] */
{
...
c-------------------------------------------------------------------*/
static void vecset(int n, double v[], /* v[1:*] */
                   int iv[],          /* iv[1:*] */
