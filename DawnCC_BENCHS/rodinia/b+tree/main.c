...
// values for each search)
// j <x> <y> -- Run a range search of <x> bundled queries on the CPU and GPU
// (B+Tree) with the range of each search of size <y>
...
void list_insert_item_sorted(list_t *l, list_item_t *i) {
  ...
  if (l->head) {
    for (itr = l->head;
         itr &&
         l->compare(list_item_get_datum(i), list_item_get_datum(itr)) < 0;
         itr = itr->next)
      ;
    if (itr) {
      i->next = itr;
