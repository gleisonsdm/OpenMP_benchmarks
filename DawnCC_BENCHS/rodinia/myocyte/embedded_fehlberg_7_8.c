...
//     and error estimate.                                                    //
//     For step i+1,                                                          //
//        y[i+1] = y[i] +  h * (41/840 * k1 + 34/105 * finavalu_temp[5] + 9/35 *
//        finavalu_temp[6]         //
//                        + 9/35 * finavalu_temp[7] + 9/280 * finavalu_temp[8] +
//                        9/280 finavalu_temp[9] + 41/840 finavalu_temp[10] ) //
//     where                                                                  //
//     k1 = f( x[i],y[i] ),                                                   //
...
//     //
//     finavalu_temp[4] = f( x[i]+5h/12, y[i]+h/48*(20 k1 - 75 finavalu_temp[2]
//     + 75 finavalu_temp[3])),                //
//     finavalu_temp[5] = f( x[i]+h/2, y[i]+h/20*( k1 + 5 finavalu_temp[3] + 4
//     finavalu_temp[4] ) ),                    //
//     finavalu_temp[6] = f( x[i]+5h/6, y[i]+h/108*( -25 k1 + 125
//     finavalu_temp[3] - 260 finavalu_temp[4] + 250 finavalu_temp[5] ) ), //
//     finavalu_temp[7] = f( x[i]+h/6, y[i]+h*( 31/300 k1 + 61/225
//     finavalu_temp[4] - 2/9 finavalu_temp[5]              //
//                                                            + 13/900
//                                                            finavalu_temp[6])
//                                                            )  //
//     finavalu_temp[8] = f( x[i]+2h/3, y[i]+h*( 2 k1 - 53/6 finavalu_temp[3] +
//     704/45 finavalu_temp[4] - 107/9 finavalu_temp[5]      //
//                                                      + 67/90 finavalu_temp[6]
//                                                      + 3 finavalu_temp[7]) ),
//                                                      //
//     finavalu_temp[9] = f( x[i]+h/3, y[i]+h*( -91/108 k1 + 23/108
//     finavalu_temp[3] - 976/135 finavalu_temp[4]        //
//                             + 311/54 finavalu_temp[5] - 19/60
//                             finavalu_temp[6] + 17/6 finavalu_temp[7] - 1/12
//                             finavalu_temp[8]) ), //
//     finavalu_temp[10] = f( x[i]+h, y[i]+h*( 2383/4100 k1 - 341/164
//     finavalu_temp[3] + 4496/1025 finavalu_temp[4]     //
//          - 301/82 finavalu_temp[5] + 2133/4100 finavalu_temp[6] + 45/82
//          finavalu_temp[7] + 45/164 finavalu_temp[8] + 18/41 finavalu_temp[9])
//          )  //
//     finavalu_temp[11] = f( x[i], y[i]+h*( 3/205 k1 - 6/41 finavalu_temp[5] -
//     3/205 finavalu_temp[6] - 3/41 finavalu_temp[7]        //
//                                                   + 3/41 finavalu_temp[8] +
//                                                   6/41 finavalu_temp[9]) ) //
//     finavalu_temp[12] = f( x[i]+h, y[i]+h*( -1777/4100 k1 - 341/164
//     finavalu_temp[3] + 4496/1025 finavalu_temp[4]    //
//                      - 289/82 finavalu_temp[5] + 2193/4100 finavalu_temp[6] +
//                      51/82 finavalu_temp[7] + 33/164 finavalu_temp[8] +   //
//                                                        12/41 finavalu_temp[9]
//                                                        + finavalu_temp[11]) )
//                                                        //
//     x[i+1] = x[i] + h.                                                     //
//                                                                            //
//     The error is estimated to be                                           //
//        err = -41/840 * h * ( k1 + finavalu_temp[10] - finavalu_temp[11] -
//        finavalu_temp[12])                         //
...
static fp embedded_fehlberg_7_8(fp timeinst, fp h, fp *initvalu, fp *finavalu,
                                fp *error, fp *parameter, int mode) {
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (initvalu + 0) > (void*) (initvalu_temp + 90))
  || ((void*) (initvalu_temp + 0) > (void*) (initvalu + 91)));
  #pragma omp target data map(to: initvalu[0:91]) map(tofrom: initvalu_temp[0:90]) if(!RST_AI1)
  ...
  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[0], mode);
  ...
  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[1], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b31 * finavalu_temp[0][i] + b32 * finavalu_temp[1][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[2], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b41 * finavalu_temp[0][i] + b43 * finavalu_temp[2][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[3], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b51 * finavalu_temp[0][i] + b53 * finavalu_temp[2][i] +
             b54 * finavalu_temp[3][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[4], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b61 * finavalu_temp[0][i] + b64 * finavalu_temp[3][i] +
             b65 * finavalu_temp[4][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[5], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b71 * finavalu_temp[0][i] + b74 * finavalu_temp[3][i] +
             b75 * finavalu_temp[4][i] + b76 * finavalu_temp[5][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[6], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b81 * finavalu_temp[0][i] + b85 * finavalu_temp[4][i] +
             b86 * finavalu_temp[5][i] + b87 * finavalu_temp[6][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[7], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b91 * finavalu_temp[0][i] + b94 * finavalu_temp[3][i] +
             b95 * finavalu_temp[4][i] + b96 * finavalu_temp[5][i] +
             b97 * finavalu_temp[6][i] + b98 * finavalu_temp[7][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[8], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b11_1 * finavalu_temp[0][i] + b11_4 * finavalu_temp[3][i] +
  ...
  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[10], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b12_1 * finavalu_temp[0][i] + b12_6 * finavalu_temp[5][i] +
             b12_7 * finavalu_temp[6][i] + b12_8 * finavalu_temp[7][i] +
             b12_9 * finavalu_temp[8][i] + b12_10 * finavalu_temp[9][i]);
  }

  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[11], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    initvalu_temp[i] =
        initvalu[i] +
        h * (b13_1 * finavalu_temp[0][i] + b13_4 * finavalu_temp[3][i] +
  ...
  master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[12], mode);
  ...
  for (i = 0; i < EQUATIONS; i++) {
    finavalu[i] = initvalu[i] +
                  h * (c_1_11 * (finavalu_temp[0][i] + finavalu_temp[10][i]) +
  ...
  for (i = 0; i < EQUATIONS; i++) {
    error[i] = fabs(err_factor * (finavalu_temp[0][i] + finavalu_temp[10][i] -
                                  finavalu_temp[11][i] - finavalu_temp[12][i]));
    // printf("Error[%d] = %f\n", i, error[i]);
  }
