...
void ParseCommandLine(int argc, char **argv) {
  ...
  while (!errflg && ((ch = getopt(argc, argv, "aCchql:d:t:s:brcLM")) != EOF)) {
    ...
    case '?':
      ...
      break;
    case 'd':
      OPT_dotfilename = optarg;
      break;
    case 't':
      OPT_texfilename = optarg;
      break;
    case 'C':
      OPT_on_cpu = true;
      break;
    case 'l':
      ...
      break;
    case 'b':
      OPT_forwardreverse = true;
      break;
    case 'r':
      OPT_reverse = true;
      break;
    case 's':
      OPT_stats_file = optarg;
      break;
    case 'c':
      OPT_forwardcoordinates = true;
      break;
    case 'L':
      OPT_showQueryLength = true;
      break;
    case 'M':
      OPT_maxmatch = true;
      break;
  ...
  if (OPT_reverse && OPT_forwardreverse) {
    fprintf(stderr,
            "ERROR: Reverse (-r) and Forward & Reverse (-b) specified\n");
    exit(1);
  }
...
int main(int argc, char *argv[]) {
  ...
  MatchContext ctx;
  if ((err = createMatchContext(&ref, &queries, 0, OPT_on_cpu, OPT_match_length,
                                OPT_stats_file, OPT_reverse, OPT_forwardreverse,
                                OPT_forwardcoordinates, OPT_showQueryLength,
                                OPT_dotfilename, OPT_texfilename, &ctx))) {
    printStringForError(err);
    exit(err);
  }
