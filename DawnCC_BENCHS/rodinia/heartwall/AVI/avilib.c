...
avi_t *AVI_open_output_file(char *filename) {
  ...
  #pragma omp target data map(tofrom: AVI_header[0:2048])
  #pragma omp target
...
#define AVIF_COPYRIGHTED 0x00020000
  ...
  // ThOe ->0
  //   OUTLONG(10000000);           /* MaxBytesPerSec, I hope this will never be
  //   used */
  OUTLONG(0);
...
//   time_t calptr;
  ...
  //   AVI->n_idx*16);
  ret = avi_add_chunk(AVI, (unsigned char *)"idx1",
                      (unsigned char *)((void *)AVI->idx), AVI->n_idx * 16);
  hasIndex = (ret == 0);
...
static int avi_write_data(avi_t *AVI, char *data, unsigned long length,
  ...
  if (audio)
    ...
    n = avi_add_index_entry(AVI, (unsigned char *)"00db",
                            ((keyframe) ? 0x10 : 0x0), AVI->pos, length);
  ...
  if (audio)
    n = avi_add_chunk(AVI, (unsigned char *)astr, (unsigned char *)data,
                      length);
  else
    n = avi_add_chunk(AVI, (unsigned char *)"00db", (unsigned char *)data,
                      length);
...
int AVI_dup_frame(avi_t *AVI) {
  ...
  if (avi_add_index_entry(AVI, (unsigned char *)"00db", 0x10, AVI->last_pos,
                          AVI->last_len))
    return -1;
  AVI->video_frames++;
...
int avi_parse_input_file(avi_t *AVI, int getIndex) {
  ...
  for (i = 0; i < hdrl_len;) {
    ...
    if (strncasecmp((char *)hdrl_data + i, "strh", 4) == 0) {
      i += 8;
      if (strncasecmp((char *)hdrl_data + i, "vids", 4) == 0 &&
          !vids_strh_seen) {
        ...
        vids_strh_seen = 1;
        lasttag = 1; /* vids */
      } else if (strncasecmp((char *)hdrl_data + i, "auds", 4) == 0 &&
                 !auds_strh_seen) {
        ...
        AVI->track[AVI->aptr].audio_bytes =
            str2ulong(hdrl_data + i + 32) * avi_sampsize(AVI, 0);
  ...
  if (idx_type == 0) {
    ...
    while (1) {
      ...
      if (((data[2] == 'd' || data[2] == 'D') &&
          ...
          ((data[2] == 'w' || data[2] == 'W') &&
           (data[3] == 'b' || data[3] == 'B'))) {
        avi_add_index_entry(AVI, (unsigned char *)data, 0,
                            lseek(AVI->fdes, 0, SEEK_CUR) - 8, n);
      }
  ...
  nvi = 0;
  #pragma omp target data map(tofrom: nai[0:8])
  #pragma omp target
  for (j = 0; j < AVI->anum; ++j)
    nai[j] = 0;
  ...
  //   fprintf(stderr, "chunks = %ld %d %s\n", AVI->track[0].audio_chunks,
  //   AVI->anum, AVI->track[0].audio_tag);
  ...
  for (j = 0; j < AVI->anum; ++j) {
    if (AVI->track[j].audio_chunks) {
      AVI->track[j].audio_index =
          (audio_index_entry *)malloc(nai[j] * sizeof(audio_index_entry));
  ...
  nvi = 0;
  char RST_AI4 = 0;
  #pragma omp target data map(tofrom: nai[0:8],tot[0:8]) if(!RST_AI4)
...
int AVI_video_width(avi_t *AVI) { return AVI->width; }
int AVI_video_height(avi_t *AVI) { return AVI->height; }
double AVI_frame_rate(avi_t *AVI) { return AVI->fps; }
char *AVI_video_compressor(avi_t *AVI) { return AVI->compressor2; }

long AVI_max_video_chunk(avi_t *AVI) { return AVI->max_len; }

int AVI_audio_tracks(avi_t *AVI) { return (AVI->anum); }

int AVI_audio_channels(avi_t *AVI) { return AVI->track[AVI->aptr].a_chans; }

long AVI_audio_mp3rate(avi_t *AVI) { return AVI->track[AVI->aptr].mp3rate; }

int AVI_audio_bits(avi_t *AVI) { return AVI->track[AVI->aptr].a_bits; }

int AVI_audio_format(avi_t *AVI) { return AVI->track[AVI->aptr].a_fmt; }

long AVI_audio_rate(avi_t *AVI) { return AVI->track[AVI->aptr].a_rate; }

long AVI_audio_bytes(avi_t *AVI) { return AVI->track[AVI->aptr].audio_bytes; }
...
long AVI_video_codech_offset(avi_t *AVI) { return AVI->v_codech_off; }

long AVI_video_codecf_offset(avi_t *AVI) { return AVI->v_codecf_off; }
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes) {
  ...
  while (bytes > 0) {
    left = AVI->track[AVI->aptr]
               .audio_index[AVI->track[AVI->aptr].audio_posc]
               .len -
           AVI->track[AVI->aptr].audio_posb;
    if (left == 0) {
...
char *(avi_errors[]) = {
    ...
    /*  5 */ (
        char *) "avilib - Error writing index (file may still be useable)",
...
void AVI_print_error(char *str) {
  ...
  aerrno = (AVI_errno >= 0 && AVI_errno < num_avi_errors) ? AVI_errno
                                                          : num_avi_errors - 1;
  ...
  if (AVI_errno == AVI_ERR_OPEN || AVI_errno == AVI_ERR_READ ||
      AVI_errno == AVI_ERR_WRITE || AVI_errno == AVI_ERR_WRITE_INDEX ||
      AVI_errno == AVI_ERR_CLOSE) {
    perror("REASON");
  }
}
...
uint64_t AVI_max_size() { return ((uint64_t)AVI_MAX_LEN); }
