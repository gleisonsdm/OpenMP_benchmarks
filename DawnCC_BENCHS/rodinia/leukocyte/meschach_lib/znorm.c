...
double _zv_norm1(x, scale) ZVEC *x;
VEC *scale;
{
  int i, dim;
  Real s, sum;
...
double _zv_norm2(x, scale) ZVEC *x;
VEC *scale;
{
  int i, dim;
  Real s, sum;
  ...
  if (scale == VNULL)
    ...
    for (i = 0; i < dim; i++) {
      s = scale->ve[i];
      sum += (s == 0.0)
                 ? square(x->ve[i].re) + square(x->ve[i].im)
...
double _zv_norm_inf(x, scale) ZVEC *x;
VEC *scale;
{
  int i, dim;
  Real s, maxval, tmp;
