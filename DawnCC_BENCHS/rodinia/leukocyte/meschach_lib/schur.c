...
#ifndef ANSI_C
static void hhldr3(x, y, z, nu1, beta, newval) double x, y, z;
Real *nu1, *beta, *newval;
#else
static void hhldr3(double x, double y, double z, Real *nu1, Real *beta,
                   Real *newval)
#endif
{
  Real alpha;
...
#ifndef ANSI_C
static void hhldr3cols(A, k, j0, beta, nu1, nu2, nu3) MAT *A;
int k, j0;
double beta, nu1, nu2, nu3;
#else
static void hhldr3cols(MAT *A, int k, int j0, double beta, double nu1,
...
#endif
{
  ...
  for (j = j0; j < n; j++) {
    ...
    *****/
    ...
    ip = nu1 * m_entry(A, k, j) + nu2 * m_entry(A, k + 1, j) +
         nu3 * m_entry(A, k + 2, j);
    prod = ip * beta;
    /*****
...
#ifndef ANSI_C
static void hhldr3rows(A, k, i0, beta, nu1, nu2, nu3) MAT *A;
int k, i0;
double beta, nu1, nu2, nu3;
#else
static void hhldr3rows(MAT *A, int k, int i0, double beta, double nu1,
                       double nu2, double nu3)
#endif
{
  Real **A_me, ip, prod;
  ...
  for (i = 0; i <= i0; i++) {
    ...
    ****/
    ...
    ip = nu1 * m_entry(A, i, k) + nu2 * m_entry(A, i, k + 1) +
         nu3 * m_entry(A, i, k + 2);
    prod = ip * beta;
...
/* treat degenerate cases differently
      ...
      ********************/
      ...
      for (k = k_min; k <= k_max - 1; k++) {
        if (k < k_max - 1) {
          ...
          tracecatch(hhldr3cols(A, k, max(k - 1, 0), beta2, nu1, y, z),
                     "schur");
          tracecatch(hhldr3rows(A, k, min(n - 1, k + 3), beta2, nu1, y, z),
                     "schur");
          if (Q != MNULL)
      ...
      for (k = k_min; k < k_max; k++)
        if (fabs(A_me[k + 1][k]) <
            MACHEPS * (fabs(A_me[k][k]) + fabs(A_me[k + 1][k + 1]))) {
          A_me[k + 1][k] = 0.0;
          split = TRUE;
        }
    }
  ...
  for (i = 0; i < A->m - 1; i++)
    if (fabs(A_me[i + 1][i]) <
        MACHEPS * (fabs(A_me[i][i]) + fabs(A_me[i + 1][i + 1])))
      A_me[i + 1][i] = 0.0;
...
#ifndef ANSI_C
void schur_evals(T, real_pt, imag_pt) MAT *T;
VEC *real_pt, *imag_pt;
#else
...
#endif
{
  ...
  while (i < n) {
    if (i < n - 1 &&
        T_me[i + 1][i] != 0.0) { /* should be a complex eigenvalue */
...
/* schur_vecs -- returns eigenvectors computed from the real Schur
        ...
        -- X_re is returned */
#ifndef ANSI_C
MAT *schur_vecs(T, Q, X_re, X_im) MAT *T, *Q, *X_re, *X_im;
#else
...
#endif
{
  ...
  Real l_re, l_im, det_re, det_im, invdet_re, invdet_im, val1_re, val1_im,
      val2_re, val2_im, tmp_val1_re, tmp_val1_im, tmp_val2_re, tmp_val2_im,
      **T_me;
  Real sum, diff, discrim, magdet, norm, scale;
  STATIC VEC *tmp1_re = VNULL, *tmp1_im = VNULL, *tmp2_re = VNULL,
             *tmp2_im = VNULL;

  if (!T || !X_re)
    error(E_NULL, "schur_vecs");
  if (T->m != T->n || X_re->m != X_re->n || (Q != MNULL && Q->m != Q->n) ||
      (X_im != MNULL && X_im->m != X_im->n))
    error(E_SQUARE, "schur_vecs");
  if (T->m != X_re->m || (Q != MNULL && T->m != Q->m) ||
  ...
  while (i < T->m) {
    ...
    while (j >= 0) {
      if (j > 0 && T->me[j][j - 1] != 0.0) { /* 2 x 2 diagonal block */
        ...
        scale = fabs(T_me[j - 1][j - 1]) + fabs(T_me[j][j]) + fabs(t12) +
                fabs(t21) + fabs(l_re) + fabs(l_im);
