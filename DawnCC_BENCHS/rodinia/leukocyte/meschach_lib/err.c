...
/* attach a new list of errors pointed by err_ptr
   ...
   */
#ifndef ANSI_C
int err_list_attach(list_num, list_len, err_ptr, warn) int list_num, list_len,
    warn;
char **err_ptr;
#else
...
#ifndef ANSI_C
int err_list_free(list_num) int list_num;
#else
...
/* check if list_num is attached;
   ...
   */
#ifndef ANSI_C
int err_is_list_attached(list_num) int list_num;
#else
...
#ifndef ANSI_C
int set_err_flag(flag) int flag;
#else
 ...
 */
#ifndef ANSI_C
int ev_err(file, err_num, line_num, fn_name, list_num) char *file, *fn_name;
int err_num, line_num, list_num;
#else
int ev_err(const char *file, int err_num, int line_num, const char *fn_name,
           int list_num)
#endif
{
  int num;
  ...
  if (cnt_errs && ++num_errs >= MAX_ERRS) /* too many errors */
  {
    fprintf(stderr, "\n\"%s\", line %d: %s in function %s()\n", file, line_num,
            err_list[list_num].listp[num], isascii(*fn_name) ? fn_name : "???");
    if (!isatty(fileno(stdout)))
      fprintf(stdout, "\n\"%s\", line %d: %s in function %s()\n", file,
              line_num, err_list[list_num].listp[num],
...
#ifndef ANSI_C
static void float_error(num) int num;
#else
...
void catch_FPE() { signal(SIGFPE, float_error); }
