...
#ifndef ANSI_C
ZMAT *_zm_copy(in, out, i0, j0) ZMAT *in, *out;
unsigned int i0, j0;
#else
...
#ifndef ANSI_C
ZVEC *_zv_copy(in, out, i0) ZVEC *in, *out;
unsigned int i0;
#else
...
#ifndef ANSI_C
ZMAT *zm_move(in, i0, j0, m0, n0, out, i1, j1) ZMAT *in, *out;
int i0, j0, m0, n0, i1, j1;
#else
ZMAT *zm_move(const ZMAT *in, int i0, int j0, int m0, int n0, ZMAT *out, int i1,
              int j1)
#endif
{
  int i;
...
#ifndef ANSI_C
ZVEC *zv_move(in, i0, dim0, out, i1) ZVEC *in, *out;
int i0, dim0, i1;
#else
...
/* zmv_move -- copies selected piece of matrix to a vector
        ...
        -- out is resized if necessary */
#ifndef ANSI_C
ZVEC *zmv_move(in, i0, j0, m0, n0, out, i1) ZMAT *in;
ZVEC *out;
int i0, j0, m0, n0, i1;
#else
ZVEC *zmv_move(const ZMAT *in, int i0, int j0, int m0, int n0, ZVEC *out,
               int i1)
#endif
{
  int dim1, i;
  ...
  if (i0 < 0 || j0 < 0 || m0 < 0 || n0 < 0 || i1 < 0 || i0 + m0 > in->m ||
      j0 + n0 > in->n)
  ...
  for (i = 0; i < m0; i++)
    MEM_COPY(&(in->me[i0 + i][j0]), &(out->ve[i1 + i * n0]),
             n0 * sizeof(complex));
...
/* zvm_move -- copies selected piece of vector to a matrix
        ...
        -- out is resized if necessary */
#ifndef ANSI_C
ZMAT *zvm_move(in, i0, out, i1, j1, m1, n1) ZVEC *in;
ZMAT *out;
int i0, i1, j1, m1, n1;
#else
ZMAT *zvm_move(const ZVEC *in, int i0, ZMAT *out, int i1, int j1, int m1,
               int n1)
#endif
{
  int dim0, i;
  ...
  dim0 = m1 * n1;
  for (i = 0; i < m1; i++)
    MEM_COPY(&(in->ve[i0 + i * n1]), &(out->me[i1 + i][j1]),
             n1 * sizeof(complex));
