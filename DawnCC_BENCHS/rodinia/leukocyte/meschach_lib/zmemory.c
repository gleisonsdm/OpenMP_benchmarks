...
#endif
{
  ...
  if (mat == (ZMAT *)NULL || (int)(mat->m) < 0 || (int)(mat->n) < 0)
    /* don't trust it */
    return (-1);
...
#endif
{
  ...
  if (vec->ve == (complex *)NULL) {
    ...
    if (mem_info_is_on()) {
      mem_bytes(TYPE_ZVEC, vec->max_dim * sizeof(complex) + sizeof(ZVEC), 0);
      mem_numvar(TYPE_ZVEC, -1);
    }
...
#ifndef ANSI_C
ZMAT *zm_resize(A, new_m, new_n) ZMAT *A;
int new_m, new_n;
#else
...
#ifndef ANSI_C
ZVEC *zv_resize(x, new_dim) ZVEC *x;
int new_dim;
#else
