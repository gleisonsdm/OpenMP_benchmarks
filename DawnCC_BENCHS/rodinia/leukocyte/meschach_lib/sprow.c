...
#endif
{
  ...
  for (j_idx = 0; j_idx < r->len; j_idx++, elts++)
    fprintf(fp, "Col: %d, Val: %g, nxt_row = %d, nxt_idx = %d\n", elts->col,
            elts->val, elts->nxt_row, elts->nxt_idx);
  fprintf(fp, "\n");
}
#endif /* MEX */
...
#ifndef ANSI_C
SPROW *sprow_get(maxlen) int maxlen;
#else
...
/* sprow_xpd -- expand row by means of realloc()
   ...
   -- returns r */
#ifndef ANSI_C
SPROW *sprow_xpd(r, n, type) SPROW *r;
int n, type;
#else
...
#endif
{
  ...
  if (n <= r->len)
    ...
    newlen = n;
  if (newlen <= r->maxlen) {
    MEM_ZERO((char *)(&(r->elt[r->len])), (newlen - r->len) * sizeof(row_elt));
    r->len = newlen;
  } else {
    if (mem_info_is_on()) {
...
/* sprow_resize -- resize a SPROW variable by means of realloc()
   ...
   -- returns r */
#ifndef ANSI_C
SPROW *sprow_resize(r, n, type) SPROW *r;
int n, type;
#else
...
/* sprow_merge -- merges r1 and r2 into r_out
   ...
   -- returns r_out */
#ifndef ANSI_C
SPROW *sprow_merge(r1, r2, r_out, type) SPROW *r1, *r2, *r_out;
int type;
#else
...
/* sprow_copy -- copies r1 and r2 into r_out
   ...
   -- returns r_out */
#ifndef ANSI_C
SPROW *sprow_copy(r1, r2, r_out, type) SPROW *r1, *r2, *r_out;
int type;
#else
...
/* sprow_mltadd -- sets r_out <- r1 + alpha.r2
   ...
   -- returns r_out */
#ifndef ANSI_C
SPROW *sprow_mltadd(r1, r2, alpha, j0, r_out, type) SPROW *r1, *r2, *r_out;
double alpha;
int j0, type;
#else
SPROW *sprow_mltadd(const SPROW *r1, const SPROW *r2, double alpha, int j0,
...
/* sprow_add -- sets r_out <- r1 + r2
   ...
   -- returns r_out */
#ifndef ANSI_C
SPROW *sprow_add(r1, r2, j0, r_out, type) SPROW *r1, *r2, *r_out;
int j0, type;
#else
SPROW *sprow_add(const SPROW *r1, const SPROW *r2, int j0, SPROW *r_out,
                 int type)
#endif
{
  int idx1, idx2, idx_out, len1, len2, len_out;
...
/* sprow_sub -- sets r_out <- r1 - r2
   ...
   -- returns r_out */
#ifndef ANSI_C
SPROW *sprow_sub(r1, r2, j0, r_out, type) SPROW *r1, *r2, *r_out;
int j0, type;
#else
SPROW *sprow_sub(const SPROW *r1, const SPROW *r2, int j0, SPROW *r_out,
                 int type)
#endif
{
  int idx1, idx2, idx_out, len1, len2, len_out;
...
/* sprow_smlt -- sets r_out <- alpha*r1
   ...
   -- returns r_out */
#ifndef ANSI_C
SPROW *sprow_smlt(r1, alpha, j0, r_out, type) SPROW *r1, *r_out;
double alpha;
int j0, type;
#else
...
#ifndef ANSI_C
void sprow_foutput(fp, r) FILE *fp;
SPROW *r;
#else
...
#ifndef ANSI_C
double sprow_set_val(r, j, val) SPROW *r;
int j;
double val;
#else
...
#endif
{
  ...
  /* else */ if (idx < -1) {
    ...
    for (idx2 = r->len - 1; idx2 >= idx; idx2--)
      MEM_COPY((char *)(&(r->elt[idx2])), (char *)(&(r->elt[idx2 + 1])),
               sizeof(row_elt));
    /************************************************************
      if ( idx < r->len )
