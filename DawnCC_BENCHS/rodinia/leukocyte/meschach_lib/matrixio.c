  ...
  redo:
    ...
    for (j = 0; j < n; j++)
      ...
      redo2:
        ...
        if (!dynamic)
          fprintf(stderr, "old %14.9g new: ", mat->me[i][j]);
        if (fgets(line, MAXLINE, fp) == NULL)
          error(E_INPUT, "im_finput");
...
#endif
{
  ...
  /* get dimension */
  skipjunk(fp);
  if ((io_code = fscanf(fp, " Matrix: %u by %u", &m, &n)) < 2 || m > MAXDIM ||
      n > MAXDIM)
...
#endif
{
  ...
  while (i < size) {
    ...
    redo:
      ...
      if (!dynamic)
        fprintf(stderr, "old: %u->%u new: ", i, px->pe[i]);
      if (fgets(line, MAXLINE, fp) == NULL)
        error(E_INPUT, "ipx_finput");
...
#endif
{
  ...
  /* get dimension */
  skipjunk(fp);
  if ((io_code = fscanf(fp, " Vector: dim:%u", &dim)) < 1 || dim > MAXDIM)
...
#ifndef ANSI_C
char *setformat(f_string) char *f_string;
#else
...
#endif
{
  ...
  /* get dimension */
  skipjunk(fp);
  if ((io_code = fscanf(fp, " IntVector: dim:%u", &dim)) < 1 || dim > MAXDIM)
