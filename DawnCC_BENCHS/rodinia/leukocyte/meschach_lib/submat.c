...
#ifndef ANSI_C
VEC *get_col(mat, col, vec) unsigned int col;
MAT *mat;
VEC *vec;
#else
...
#ifndef ANSI_C
VEC *get_row(mat, row, vec) unsigned int row;
MAT *mat;
VEC *vec;
#else
...
#ifndef ANSI_C
MAT *_set_col(mat, col, vec, i0) MAT *mat;
VEC *vec;
unsigned int col, i0;
#else
...
#ifndef ANSI_C
MAT *_set_row(mat, row, vec, j0) MAT *mat;
VEC *vec;
unsigned int row, j0;
#else
...
#ifndef ANSI_C
MAT *sub_mat(old, row1, col1, row2, col2, new) MAT *old, *new;
unsigned int row1, col1, row2, col2;
#else
MAT *sub_mat(const MAT *old, unsigned int row1, unsigned int col1,
             unsigned int row2, unsigned int col2, MAT *new)
#endif
{
  unsigned int i;
  ...
  if (new == (MAT *)NULL || new->m < row2 - row1 + 1) {
    ...
    if (new == (MAT *)NULL || new->me == (Real **)NULL)
      ...
      mem_bytes(TYPE_MAT, 0, sizeof(MAT) + (row2 - row1 + 1) * sizeof(Real *));
    }
  }
  new->m = row2 - row1 + 1;
...
#ifndef ANSI_C
VEC *sub_vec(old, i1, i2, new) VEC *old, *new;
int i1, i2;
#else
