...
#ifndef ANSI_C
VEC *hhvec(vec, i0, beta, out, newval) VEC *vec, *out;
unsigned int i0;
Real *beta, *newval;
#else
VEC *hhvec(const VEC *vec, unsigned int i0, Real *beta, VEC *out, Real *newval)
#endif
{
  Real norm;
...
/* hhtrvec -- apply Householder transformation to vector
        ...
        -- may be in-situ */
#ifndef ANSI_C
VEC *hhtrvec(hh, beta, i0, in, out) VEC *hh, *in,
    *out; /* hh = Householder vector */
unsigned int i0;
double beta;
#else
...
#ifndef ANSI_C
MAT *hhtrrows(M, i0, j0, hh, beta) MAT *M;
unsigned int i0, j0;
VEC *hh;
double beta;
#else
MAT *hhtrrows(MAT *M, unsigned int i0, unsigned int j0, const VEC *hh,
              double beta)
#endif
{
  Real ip, scale;
...
*/
#ifndef ANSI_C
MAT *hhtrcols(M, i0, j0, hh, beta) MAT *M;
unsigned int i0, j0;
VEC *hh;
double beta;
#else
MAT *hhtrcols(MAT *M, unsigned int i0, unsigned int j0, const VEC *hh,
              double beta)
#endif
{
  STATIC VEC *w = VNULL;
...
*/
#ifndef ANSI_C
MAT *_hhtrcols(M, i0, j0, hh, beta, w) MAT *M;
unsigned int i0, j0;
VEC *hh;
double beta;
VEC *w;
#else
MAT *_hhtrcols(MAT *M, unsigned int i0, unsigned int j0, const VEC *hh,
               double beta, VEC *w)
#endif
{
  /* Real	ip, scale; */
