...
ZVEC *zUsolve(matrix, b, out, diag) ZMAT *matrix;
ZVEC *b, *out;
double diag;
{
  unsigned int dim /* , j */;
...
ZVEC *zLsolve(matrix, b, out, diag) ZMAT *matrix;
ZVEC *b, *out;
double diag;
{
...
ZVEC *zUAsolve(U, b, out, diag) ZMAT *U;
ZVEC *b, *out;
double diag;
{
  unsigned int dim, i, i_lim /* , j */;
  ...
  if (diag == 0.0) {
    for (; i < dim; i++) {
      ...
      tmp.re = -out_ve[i].re;
      tmp.im = -out_ve[i].im;
      __zmltadd__(&(out_ve[i + 1]), &(U_me[i][i + 1]), tmp, dim - i - 1,
                  Z_CONJ);
    }
  } else {
    invdiag = 1.0 / diag;
...
/* zLAsolve -- back substitution with optional over-riding diagonal
                ...
                -- can be in-situ but doesn't need to be */
ZVEC *zLAsolve(L, b, out, diag) ZMAT *L;
ZVEC *b, *out;
double diag;
{
  unsigned int dim;
