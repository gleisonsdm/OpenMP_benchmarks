...
void zgivens(x, y, c, s) complex x, y, *s;
Real *c;
{
  Real inv_norm, norm;
  complex tmp;
...
ZVEC *rot_zvec(x, i, k, c, s, out) ZVEC *x, *out;
int i, k;
double c;
complex s;
{
  ...
  temp1.re = c * out->ve[i].re - s.re * out->ve[k].re + s.im * out->ve[k].im;
  temp1.im = c * out->ve[i].im - s.re * out->ve[k].im - s.im * out->ve[k].re;

  /* temp2 = c*out->ve[k] + zconj(s)*out->ve[i]; */
  temp2.re = c * out->ve[k].re + s.re * out->ve[i].re + s.im * out->ve[i].im;
  temp2.im = c * out->ve[k].im + s.re * out->ve[i].im - s.im * out->ve[i].re;
...
ZMAT *zrot_rows(mat, i, k, c, s, out) ZMAT *mat, *out;
int i, k;
double c;
complex s;
{
  ...
  for (j = 0; j < mat->n; j++) {
    /* temp1 = c*out->me[i][j] - s*out->me[k][j]; */
    temp1.re = c * out->me[i][j].re - s.re * out->me[k][j].re +
               s.im * out->me[k][j].im;
    temp1.im = c * out->me[i][j].im - s.re * out->me[k][j].im -
               s.im * out->me[k][j].re;

    /* temp2 = c*out->me[k][j] + conj(s)*out->me[i][j]; */
    temp2.re = c * out->me[k][j].re + s.re * out->me[i][j].re +
               s.im * out->me[i][j].im;
    temp2.im = c * out->me[k][j].im + s.re * out->me[i][j].im -
               s.im * out->me[i][j].re;
...
ZMAT *zrot_cols(mat, i, k, c, s, out) ZMAT *mat, *out;
int i, k;
double c;
complex s;
{
