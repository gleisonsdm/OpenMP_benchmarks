...
/* cg_set_maxiter -- sets maximum number of iterations if numiter > 1
        ...
        -- returns old maximum */
int cg_set_maxiter(numiter) int numiter;
{
  int temp;
...
VEC *pccg(A, A_params, M_inv, M_params, b, eps, x) MTX_FN A, M_inv;
VEC *b, *x;
double eps;
...
VEC *sp_pccg(A, LLT, b, eps, x) SPMAT *A, *LLT;
VEC *b, *x;
double eps;
...
VEC *cgs(A, A_params, b, r0, tol, x) MTX_FN A;
VEC *x, *b;
...
VEC *sp_cgs(A, b, r0, tol, x) SPMAT *A;
VEC *b, *r0, *x;
double tol;
...
/* lsqr -- sparse CG-like least squares routine:
        ...
        -- returns x (if x != NULL) */
VEC *lsqr(A, AT, A_params, b, tol, x) MTX_FN A, AT; /* AT is A transposed */
VEC *x, *b;
double tol; /* error tolerance used */
void *A_params;
{
  VEC *u, *v, *w, *tmp;
  Real alpha, beta, norm_b, phi, phi_bar, rho, rho_bar, rho_max, theta;
