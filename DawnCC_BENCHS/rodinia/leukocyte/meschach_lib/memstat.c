...
/* hashing function */
#ifndef ANSI_C
static unsigned int mem_hash(ptr) void **ptr;
#else
...
#ifndef ANSI_C
static int mem_lookup(var) void **var;
#else
...
#endif
{
  ...
  if (mem_stat_var[k].var == var) {
    ...
    if (mem_stat_var[j].var == NULL)
      ...
      j = 0;
      while (mem_stat_var[j].var != var && j < k && mem_stat_var[j].var != NULL)
        j++;
      if (mem_stat_var[j].var == NULL)
        return j;
      else if (mem_stat_var[j].var == var)
...
*/
#ifndef ANSI_C
int mem_stat_reg_list(var, type, list, fname, line) void **var;
int type, list;
char *fname;
int line;
#else
...
*/
#ifndef ANSI_C
int mem_stat_mark(mark) int mark;
#else
...
*/
#ifndef ANSI_C
int mem_stat_free_list(mark, list) int mark, list;
#else
...
#endif
{
  ...
  #pragma omp target data map(to: mem_hash_idx[0:509]) 
  #pragma omp target
...
#ifndef ANSI_C
void mem_stat_dump(fp, list) FILE *fp;
int list;
#else
