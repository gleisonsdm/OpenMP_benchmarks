...
#ifndef ANSI_C
double sp_get_val(A, i, j) SPMAT *A;
int i, j;
#else
...
#ifndef ANSI_C
double sp_set_val(A, i, j, val) SPMAT *A;
int i, j;
double val;
#else
...
#ifndef ANSI_C
SPMAT *sp_get(m, n, maxlen) int m, n, maxlen;
#else
...
#ifndef ANSI_C
SPMAT *sp_diag_access(A) SPMAT *A;
#else
...
#ifndef ANSI_C
SPMAT *sp_add(A, B, C) SPMAT *A, *B, *C;
#else
...
#ifndef ANSI_C
SPMAT *sp_sub(A, B, C) SPMAT *A, *B, *C;
#else
...
#ifndef ANSI_C
SPMAT *sp_mltadd(A, B, alpha, C) SPMAT *A, *B, *C;
double alpha;
#else
...
#endif
{
  ...
  if (in_situ)
    ...
    for (i = 0; i < A->m; i++) {
      sprow_mltadd(&(A->row[i]), &(B->row[i]), alpha, 0, &(C->row[i]),
                   TYPE_SPMAT);
    }
...
#ifndef ANSI_C
SPMAT *sp_smlt(A, alpha, B) SPMAT *A, *B;
double alpha;
#else
...
#ifndef ANSI_C
SPMAT *sp_copy2(A, OUT) SPMAT *A, *OUT;
#else
...
#ifndef ANSI_C
SPMAT *sp_compact(A, tol) SPMAT *A;
double tol;
#else
...
/* sp_mlt (C) Copyright David Stewart and Fabrizio Novalis
 * <novalis@mars.elet.polimi.it> */
