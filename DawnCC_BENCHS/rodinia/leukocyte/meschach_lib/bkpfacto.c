...
static void interchange(A, i, j) MAT *A; /* assumed != NULL & also SQUARE */
...
#ifndef ANSI_C
MAT *BKPfactor(A, pivot, blocks) MAT *A;
PERM *pivot, *blocks;
#else
...
#endif
{
  ...
  for (i = 0; i < n; i = onebyone ? i + 1 : i + 2) {
    ...
    for (k = i; k < n; k++) {
      if (k == r)
        continue;
      tmp = (k > r) ? fabs(m_entry(A, r, k)) : fabs(m_entry(A, k, r));
      if (tmp > sigma)
        sigma = tmp;
    }
  ...
  dopivot:
    ...
    {      /* do two by two block */
      det = m_entry(A, i, i) * m_entry(A, i + 1, i + 1) -
            sqr(m_entry(A, i, i + 1));
      /* Must have det < 0 */
      /* printf("# det = %g\n",det); */
...
#ifndef ANSI_C
VEC *BKPsolve(A, pivot, block, b, x) MAT *A;
PERM *pivot, *block;
VEC *b, *x;
#else
VEC *BKPsolve(const MAT *A, PERM *pivot, const PERM *block, const VEC *b,
              VEC *x)
#endif
{
  ...
  for (i = n - 1; i >= 0;
       i--) { /* use symmetry of factored form to get stride 1 */
    sum = v_entry(tmp, i);
    if (block->pe[i] > i)
      for (j = i + 2; j < n; j++)
        sum -= m_entry(A, i, j) * v_entry(tmp, j);
