...
int m3d_free(mat) MAT3D *mat;
{
  ...
  if (mat->base != (Real *)NULL) {
    if (mem_info_is_on())
      ...
      mem_bytes_list(TYPE_MAT3D,
                     mat->max_l * mat->max_m * mat->max_n * sizeof(Real), 0,
                     M3D_LIST);
    free((char *)mat->base);
  }
  ...
  if (mat->me2d != (Real **)NULL) {
    if (mem_info_is_on())
      /* record how many bytes is deallocated */
      mem_bytes_list(TYPE_MAT3D, mat->max_l * mat->max_m * sizeof(Real *), 0,
                     M3D_LIST);
    free((char *)mat->me2d);
  }
