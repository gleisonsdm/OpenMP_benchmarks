...
#include "zmatrix.h"
static char rcsid[] =
    "$Id: zvecop.c,v 1.3 1997/10/07 16:13:54 stewart Exp stewart $";

/* _zin_prod -- inner product of two vectors from i0 downwards
        -- flag != 0 means compute sum_i a[i]*.b[i];
        -- flag == 0 means compute sum_i a[i].b[i] */
#ifndef ANSI_C
complex _zin_prod(a, b, i0, flag) ZVEC *a, *b;
unsigned int i0, flag;
#else
complex _zin_prod(const ZVEC *a, const ZVEC *b, unsigned int i0,
                  unsigned int flag)
#endif
{
  unsigned int limit;
...
#ifndef ANSI_C
ZVEC *zv_mlt(scalar, vector, out) complex scalar;
ZVEC *vector, *out;
#else
...
#ifndef ANSI_C
ZVEC *zv_add(vec1, vec2, out) ZVEC *vec1, *vec2, *out;
#else
...
/* zv_mltadd -- scalar/vector multiplication and addition
                -- out = v1 + scale.v2		*/
#ifndef ANSI_C
ZVEC *zv_mltadd(v1, v2, scale, out) ZVEC *v1, *v2, *out;
complex scale;
#else
...
#ifndef ANSI_C
ZVEC *zv_sub(vec1, vec2, out) ZVEC *vec1, *vec2, *out;
#else
...
void *params;
#else
ZVEC *_zv_map(complex (*f)(void *, complex), void *params, const ZVEC *x,
              ZVEC *out)
#endif
{
  complex *x_ve, *out_ve;
...
#ifndef ANSI_C
ZVEC *zv_lincomb(n, v, a, out) int n; /* number of a's and v's */
complex a[];
ZVEC *v[], *out;
#else
...
#ifndef ANSI_C
ZVEC *zv_star(x1, x2, out) ZVEC *x1, *x2, *out;
#else
...
/* zv_slash -- computes componentwise ratio of x2 and x1
        ...
        -- result out is returned */
#ifndef ANSI_C
ZVEC *zv_slash(x1, x2, out) ZVEC *x1, *x2, *out;
#else
...
#ifndef ANSI_C
ZVEC *px_zvec(px, vector, out) PERM *px;
ZVEC *vector, *out;
#else
...
/* pxinv_zvec -- apply the inverse of px to x, returning the result in out
                -- may NOT be in situ */
#ifndef ANSI_C
ZVEC *pxinv_zvec(px, x, out) PERM *px;
ZVEC *x, *out;
#else
