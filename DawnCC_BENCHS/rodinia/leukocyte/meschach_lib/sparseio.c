...
#endif
{
  ...
  for (i = 0; i < m; i++) {
    ...
    for (j_idx = 0; j_idx < rows[i].len; j_idx++) {
      fprintf(fp, "%d:%-20.15g ", elts[j_idx].col, elts[j_idx].val);
...
#endif
{
  ...
  for (i = 0; i < A->m; i++) {
    fprintf(fp, "row %d: len = %d, maxlen = %d, diag idx = %d\n", i,
            rows[i].len, rows[i].maxlen, rows[i].diag);
  ...
  if (tty) {
    ...
    for (i = 0; i < m; i++) {
      ...
      if (len > 5) {
        ...
        rows[i].elt =
            (row_elt *)realloc((char *)rows[i].elt, len * sizeof(row_elt));
        rows[i].maxlen = len;
      }
