...
#ifndef ANSI_C
double __ip__(dp1, dp2, len) register Real *dp1, *dp2;
int len;
#else
...
#endif
{
  ...
  #pragma omp target data map(to: dp1[0:AI1[5]],dp2[0:AI1[5]])  if(!RST_AI1)
...
#ifndef ANSI_C
void __mltadd__(dp1, dp2, s, len) register Real *dp1, *dp2;
register double s;
register int len;
#else
...
#endif
{
  ...
  RST_AI1 |= !(((void*) (dp1 + 0) > (void*) (dp2 + AI1[5]))
  || ((void*) (dp2 + 0) > (void*) (dp1 + AI1[5])));
  #pragma omp target data map(to: dp2[0:AI1[5]]) map(tofrom: dp1[0:AI1[5]]) if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < len; i++)
    dp1[i] += s * dp2[i];
}
...
#ifndef ANSI_C
void __smlt__(dp, s, out, len) register Real *dp, *out;
register double s;
register int len;
#else
...
#endif
{
  ...
  #pragma omp target data map(to: dp[0:AI1[5]]) map(tofrom: out[0:AI1[5]]) if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < len; i++)
    out[i] = s * dp[i];
}
...
#ifndef ANSI_C
void __add__(dp1, dp2, out, len) register Real *dp1, *dp2, *out;
register int len;
#else
...
#endif
{
  ...
  #pragma omp target data map(to: dp1[0:AI1[5]],dp2[0:AI1[5]]) map(tofrom: out[0:AI1[5]]) if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < len; i++)
    out[i] = dp1[i] + dp2[i];
}
...
#ifndef ANSI_C
void __sub__(dp1, dp2, out, len) register Real *dp1, *dp2, *out;
register int len;
#else
...
#ifndef ANSI_C
void __zero__(dp, len) register Real *dp;
register int len;
#else
