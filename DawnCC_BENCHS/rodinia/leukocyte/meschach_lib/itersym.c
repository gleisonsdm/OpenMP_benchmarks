...
/* iter_spcg -- a simple interface to iter_cg() which uses sparse matrix
   ...
   */
#ifndef ANSI_C
VEC *iter_spcg(A, LLT, b, eps, x, limit, steps) SPMAT *A, *LLT;
VEC *b, *x;
double eps;
int *steps, limit;
#else
...
#endif
{
  ...
  for (ip->steps = 0; ip->steps <= ip->limit; ip->steps++) {
    ...
    if (ip->info)
      ip->info(ip, nres, r, rr);
    if (ip->steps == 0)
      ip->init_res = nres;
    if (ip->stop_crit(ip, nres, r, rr))
      break;
...
#ifndef ANSI_C
void iter_lanczos(ip, a, b, beta2, Q) ITER *ip;
VEC *a, *b;
Real *beta2;
MAT *Q;
#else
...
#ifndef ANSI_C
void iter_splanczos(A, m, x0, a, b, beta2, Q) SPMAT *A;
int m;
VEC *x0, *a, *b;
Real *beta2;
MAT *Q;
...
#else
extern double frexp(double num, int *exponent), ldexp(double num, int exponent);
#endif

/* product -- returns the product of a long list of numbers
   -- answer stored in mant (mantissa) and expt (exponent) */
#ifndef ANSI_C
static double product(a, offset, expt) VEC *a;
double offset;
int *expt;
#else
  ...
  *expt = 0;
  if (offset == 0.0)
    ...
    for (i = 0; i < a->dim; i++) {
      ...
      tmp_fctr += (tmp_fctr > 0.0) ? -MACHEPS * offset : MACHEPS * offset;
...
/* product2 -- returns the product of a long list of numbers (except the k'th)
   -- answer stored in mant (mantissa) and expt (exponent) */
#ifndef ANSI_C
static double product2(a, k, expt) VEC *a;
...
#ifndef ANSI_C
static int dbl_cmp(x, y) Real *x, *y;
#else
...
#ifndef ANSI_C
VEC *iter_lanczos2(ip, evals, err_est) ITER *ip; /* ITER structure */
...
#endif
{
  ...
  if (err_est) {
    ...
    for (i = 0; i < a->dim; i++) {
      ...
      err_est->ve[i] =
          fabs(beta * ldexp(pb_mant / det_mant, pb_expt - det_expt));
    }
  }
...
/* iter_splanczos2 -- version of iter_lanczos2() that uses sparse matrix data
   structure */
#ifndef ANSI_C
VEC *iter_splanczos2(A, m, x0, evals, err_est) SPMAT *A;
int m;
