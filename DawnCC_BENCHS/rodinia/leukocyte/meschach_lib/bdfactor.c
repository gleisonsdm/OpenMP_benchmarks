...
*/
#ifndef ANSI_C
BAND *bd_get(lb, ub, n) int lb, ub, n;
#else
...
/* resize band matrix */
#ifndef ANSI_C
BAND *bd_resize(A, new_lb, new_ub, new_n) BAND *A;
...
/* copy a square matrix to a band matrix with
   lb subdiagonals and ub superdiagonals */
#ifndef ANSI_C
BAND *mat2band(A, lb, ub, bA) BAND *bA;
MAT *A;
int lb, ub;
#else
...
*/
#ifndef ANSI_C
BAND *bd_transp(in, out) BAND *in, *out;
#else
...
#endif
{
  ...
  if (!in_situ) {
    ...
    for (i = 0, l = lub; i < (lub + 1) / 2; i++, l--) {
      ubi = i - ub;
      for (j = n1 - max(lb - l, 0), jj = n1 - max(-ubi, 0), p = n1 - lb + i,
          pp = n1;
           p >= 0; j--, jj--, pp--, p--) {
        in_v[i][jj] = in_v[l][j];
        in_v[l][pp] = in_v[i][p];
...
#ifndef ANSI_C
VEC *bdv_mltadd(x, y, bA, s, out) BAND *bA;
VEC *x, *y;
double s;
VEC *out;
#else
VEC *bdv_mltadd(const VEC *x, const VEC *y, const BAND *bA, double s, VEC *out)
#endif
{
  int i, j;
...
#ifndef ANSI_C
VEC *vbd_mltadd(x, y, bA, s, out) BAND *bA;
VEC *x, *y;
double s;
VEC *out;
#else
VEC *vbd_mltadd(const VEC *x, const VEC *y, const BAND *bA, double s, VEC *out)
#endif
{
  int i, j;
...
#ifndef ANSI_C
BAND *bds_mltadd(A, B, alpha, OUT) BAND *A, *B, *OUT;
Real alpha;
#else
...
#endif
{
  ...
  for (i = 0; i <= B->lb + B->ub; i++)
    __mltadd__(OUT->mat->me[i + OUT->lb - B->lb], B->mat->me[i], alpha,
               B->mat->n);
...
/* bdLUfactor -- gaussian elimination with partial pivoting
   ...
   */
#ifndef ANSI_C
BAND *bdLUfactor(bA, pivot) BAND *bA;
PERM *pivot;
#else
...
#ifndef ANSI_C
VEC *bdLUsolve(bA, pivot, b, x) BAND *bA;
PERM *pivot;
VEC *b, *x;
#else
