...
#ifndef MEM_COPY
void MEM_COPY(from, to, len) char *from, *to;
int len;
{
  int i;
...
#ifndef MEM_ZERO
void MEM_ZERO(ptr, len) char *ptr;
int len;
{
  int i;
...
void Mscale(len, alpha, x) int len;
double alpha;
Real *x;
{
  register int i;
  ...
  AI1[4] = (AI1[3] > 0);
  AI1[5] = (AI1[4] ? AI1[3] : 0);
  #pragma omp target data map(tofrom: x[0:AI1[5]])
  #pragma omp target
  for (i = 0; i < len; i++)
    x[i] *= alpha;
}
...
Real *x, *y;
{
  ...
  #pragma omp target data map(tofrom: x[0:AI1[5]],y[0:AI1[5]]) if(!RST_AI1)
...
Real *x, *y;
{
  ...
  RST_AI1 |= !(((void*) (x + 0) > (void*) (y + AI1[5]))
  || ((void*) (y + 0) > (void*) (x + AI1[5])));
  #pragma omp target data map(to: x[0:AI1[5]]) map(tofrom: y[0:AI1[5]]) if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < len; i++)
    y[i] = x[i];
}
...
/* Maxpy -- y <- y + alpha.x */
void Maxpy(len, alpha, x, y) int len;
double alpha;
Real *x, *y;
{
  register int i, len4;
...
****************************************/
  ...
  AI1[16] = AI1[5] > AI1[7];
  AI1[17] = (AI1[16] ? AI1[5] : AI1[7]);
  AI1[18] = AI1[4] > AI1[17];
  AI1[19] = (AI1[18] ? AI1[4] : AI1[17]);
  AI1[20] = AI1[4] > AI1[19];
  AI1[21] = (AI1[20] ? AI1[4] : AI1[19]);
  AI1[22] = AI1[3] > AI1[21];
  AI1[23] = (AI1[22] ? AI1[3] : AI1[21]);
  AI1[24] = AI1[3] > AI1[23];
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (x + 0) > (void*) (y + AI1[29]))
  || ((void*) (y + 0) > (void*) (x + AI1[15])));
  #pragma omp target data map(to: x[0:AI1[15]]) map(tofrom: y[0:AI1[29]]) if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < len4; i++) {
    y[4 * i] += alpha * x[4 * i];
  ...
  char RST_AI2 = 0;
  RST_AI2 |= !(((void*) (x + AI2[18]) > (void*) (y + AI2[19]))
  || ((void*) (y + AI2[18]) > (void*) (x + AI2[19])));
  #pragma omp target data map(to: x[AI2[18]:AI2[19]]) map(tofrom: y[AI2[18]:AI2[19]]) if(!RST_AI2)
...
Real *x, *y;
{
  ...
  AI1[15] = (AI1[14] ? AI1[13] : 0);
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (x + 0) > (void*) (y + AI1[15]))
  || ((void*) (y + 0) > (void*) (x + AI1[15])));
  #pragma omp target data map(to: x[0:AI1[15]],y[0:AI1[15]])  if(!RST_AI1)
  #pragma omp target if(!RST_AI1)
  for (i = 0; i < len4; i++) {
    sum0 += x[4 * i] * y[4 * i];
  ...
  RST_AI2 |= !(((void*) (x + AI2[18]) > (void*) (y + AI2[19]))
  || ((void*) (y + AI2[18]) > (void*) (x + AI2[19])));
  #pragma omp target data map(to: x[AI2[18]:AI2[19]],y[AI2[18]:AI2[19]])  if(!RST_AI2)
  #pragma omp target if(!RST_AI2)
  for (i = 0; i < len; i++)
    sum += x[i] * y[i];
...
Real *x;
{
  ...
  AI1[4] = (AI1[3] > 0);
  AI1[5] = (AI1[4] ? AI1[3] : 0);
  #pragma omp target data map(to: x[0:AI1[5]]) 
  #pragma omp target
  for (i = 0; i < len; i++) {
    tmp = ABS(x[i]);
    if (max_val < tmp)
...
void Mmv(m, n, alpha, A, j0, x, beta, y) int m, n, j0;
double alpha, beta;
Real **A, *x, *y;
{
  ...
  ****************************************/
  ...
  for (i = 0; i < m4; i++) {
    ...
    for (j = 0; j < n4; j++) {
      ...
      tmp2 = x[4 * j + 2];
      tmp3 = x[4 * j + 3];
      sum0 = sum0 + dp0[j] * tmp0 + dp0[j + 1] * tmp1 + dp0[j + 2] * tmp2 +
             dp0[j + 3] * tmp3;
      sum1 = sum1 + dp1[j] * tmp0 + dp1[j + 1] * tmp1 + dp1[j + 2] * tmp2 +
             dp1[j + 3] * tmp3;
      sum2 = sum2 + dp2[j] * tmp0 + dp2[j + 1] * tmp1 + dp2[j + 2] * tmp2 +
             dp2[j + 3] * tmp3;
      sum3 = sum3 + dp3[j] * tmp0 + dp3[j + 1] * tmp2 + dp3[j + 2] * tmp2 +
             dp3[j + 3] * tmp3;
    }
    for (j = 0; j < n; j++) {
  ...
  for (i = 0; i < m; i++)
    y[4 * m4 + i] =
        beta * y[i] + alpha * Mdot(4 * n4 + n, &(A[4 * m4 + i][j0]), x);
}

/* Mvm -- y <- alpha.A^T.x + beta.y */
void Mvm(m, n, alpha, A, j0, x, beta, y) int m, n, j0;
double alpha, beta;
Real **A, *x, *y;
{
...
void Mupdate(m, n, alpha, x, y, A, j0) int m, n, j0;
double alpha;
Real **A, *x, *y;
{
  register int i, j, n4;
...
/* Mmm -- C <- C + alpha.A.B */
void Mmm(m, n, p, alpha, A, Aj0, B, Bj0, C, Cj0) int m, n, p; /* C is m x n */
double alpha;
Real **A, **B, **C;
int Aj0, Bj0, Cj0;
{
...
void Mmtrm(m, n, p, alpha, A, Aj0, B, Bj0, C, Cj0) int m, n, p; /* C is m x n */
double alpha;
Real **A, **B, **C;
int Aj0, Bj0, Cj0;
{
...
void Mmmtr(m, n, p, alpha, A, Aj0, B, Bj0, C, Cj0) int m, n, p; /* C is m x n */
double alpha;
Real **A, **B, **C;
int Aj0, Bj0, Cj0;
{
...
void Mmtrmtr(m, n, p, alpha, A, Aj0, B, Bj0, C, Cj0) int m, n,
    p; /* C is m x n */
double alpha;
Real **A, **B, **C;
int Aj0, Bj0, Cj0;
