...
#ifndef ANSI_C
static void fixsvd(d, U, V) VEC *d;
MAT *U, *V;
#else
...
#endif
{
  ...
  while (i_min < n) /* outer while loop */
  {
    ...
    while (!split) {
      ...
      diff = (t11 - t22) / 2;
      shift =
          t22 - t12 * t12 / (diff + sgn(diff) * sqrt(diff * diff + t12 * t12));

      /* initial Givens' rotation */
      givens(d_ve[i_min] * d_ve[i_min] - shift, d_ve[i_min] * f_ve[i_min], &c,
             &s);
      ...
      for (i = i_min; i < i_max; i++)
        if (fabs(f_ve[i]) < MACHEPS * (fabs(d_ve[i]) + fabs(d_ve[i + 1]))) {
          split = TRUE;
          f_ve[i] = 0.0;
