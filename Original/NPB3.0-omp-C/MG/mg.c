...
static void psinv(double ***r, double ***u, int n1, int n2, int n3, double c[4],
                  int k);
static void resid(double ***u, double ***v, double ***r, int n1, int n2, int n3,
                  double a[4], int k);
static void rprj3(double ***r, int m1k, int m2k, int m3k, double ***s, int m1j,
                  int m2j, int m3j, int k);
static void interp(double ***z, int mm1, int mm2, int mm3, double ***u, int n1,
                   int n2, int n3, int k);
static void norm2u3(double ***r, int n1, int n2, int n3, double *rnm2,
...
static void bubble(double ten[M][2], int j1[M][2], int j2[M][2], int j3[M][2],
                   int m, int ind);
...
#pragma omp master
  ...
  c_print_results("MG", Class, nx[lt], ny[lt], nz[lt], nit, nthreads, t, mflops,
  ...
  *n3 = nz[lt] + 2;
  ...
  if (debug_vec[1] >= 1) {
    ...
    printf("%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d\n", lt, nx[lt], ny[lt],
           nz[lt], *n1, *n2, *n3, is1, is2, is3, ie1, ie2, ie3);
  }
}
...
#pragma omp parallel for default(shared) private(i1, i2, i3, r1, r2)
  for (i3 = 1; i3 < n3 - 1; i3++) {
    for (i2 = 1; i2 < n2 - 1; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
        r1[i1] = r[i3][i2 - 1][i1] + r[i3][i2 + 1][i1] + r[i3 - 1][i2][i1] +
                 r[i3 + 1][i2][i1];
...
#pragma omp parallel for default(shared) private(i1, i2, i3, u1, u2)
  for (i3 = 1; i3 < n3 - 1; i3++) {
    for (i2 = 1; i2 < n2 - 1; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
        u1[i1] = u[i3][i2 - 1][i1] + u[i3][i2 + 1][i1] + u[i3 - 1][i2][i1] +
                 u[i3 + 1][i2][i1];
...
#pragma omp parallel for default(shared) private(j1, j2, j3, i1, i2, i3, x1,   \
                                                 y1, x2, y2)
  for (j3 = 1; j3 < m3j - 1; j3++) {
    ...
    for (j2 = 1; j2 < m2j - 1; j2++) {
      ...
      for (j1 = 1; j1 < m1j; j1++) {
        i1 = 2 * j1 - d1;
        /*C             i1 = 2*j1-1*/
        x1[i1] = r[i3 + 1][i2][i1] + r[i3 + 1][i2 + 2][i1] + r[i3][i2 + 1][i1] +
                 r[i3 + 2][i2 + 1][i1];
        y1[i1] = r[i3][i2][i1] + r[i3 + 2][i2][i1] + r[i3][i2 + 2][i1] +
                 r[i3 + 2][i2 + 2][i1];
      }
      ...
      for (j1 = 1; j1 < m1j - 1; j1++) {
        i1 = 2 * j1 - d1;
        /*C             i1 = 2*j1-1*/
        y2 = r[i3][i2][i1 + 1] + r[i3 + 2][i2][i1 + 1] + r[i3][i2 + 2][i1 + 1] +
             r[i3 + 2][i2 + 2][i1 + 1];
        ...
        for (i2 = 1; i2 <= mm2 - 1; i2++) {
          ...
          for (i1 = 1; i1 <= mm1 - 1; i1++) {
            u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] =
                u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] +
                0.125 * (z[i3][i2][i1] + z[i3][i2 - 1][i1] + z[i3][i2][i1 - 1] +
                         z[i3][i2 - 1][i1 - 1] + z[i3 - 1][i2][i1] +
                         z[i3 - 1][i2 - 1][i1] + z[i3 - 1][i2][i1 - 1] +
                         z[i3 - 1][i2 - 1][i1 - 1]);
          }
        }
      }
...
#pragma omp parallel for default(shared) private(i1, i2, i3, a) reduction(     \
    + : s) reduction(max : tmp)
  for (i3 = 1; i3 < n3 - 1; i3++) {
    for (i2 = 1; i2 < n2 - 1; i2++) {
      for (i1 = 1; i1 < n1 - 1; i1++) {
