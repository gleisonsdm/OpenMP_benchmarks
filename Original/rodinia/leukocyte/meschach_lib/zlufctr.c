...
ZVEC *zLUsolve(A, pivot, b, x) ZMAT *A;
PERM *pivot;
ZVEC *b, *x;
{
...
ZVEC *zLUAsolve(LU, pivot, b, x) ZMAT *LU;
PERM *pivot;
ZVEC *b, *x;
{
  if (!LU || !b || !pivot)
...
ZMAT *zm_inverse(A, out) ZMAT *A, *out;
{
  int i;
...
double zLUcondest(LU, pivot) ZMAT *LU;
PERM *pivot;
{
