...
#ifndef ANSI_C
double unord_get_val(A, i, j) SPMAT *A;
int i, j;
#else
...
#ifndef ANSI_C
static SPMAT *bkp_swap_elt(A, i1, j1, idx1, i2, j2, idx2) SPMAT *A;
int i1, j1, idx1, i2, j2, idx2;
#else
static SPMAT *bkp_swap_elt(SPMAT *A, int i1, int j1, int idx1, int i2, int j2,
                           int idx2)
#endif
{
  int tmp_row, tmp_idx;
  ...
  if (i1 < 0 || j1 < 0 || i2 < 0 || j2 < 0 || i1 >= A->m || j1 >= A->n ||
      i2 >= A->m || j2 >= A->n) {
  ...
  if (idx1 < 0) /* assume not allocated */
  {
    idx1 = r1->len;
    if (idx1 >= r1->maxlen) {
      tracecatch(sprow_xpd(r1, 2 * r1->maxlen + 1, TYPE_SPMAT), "bkp_swap_elt");
    }
    r1->len = idx1 + 1;
    r1->elt[idx1].col = j1;
    r1->elt[idx1].val = 0.0;
  ...
  if (idx2 < 0) {
    idx2 = r2->len;
    if (idx2 >= r2->maxlen) {
      tracecatch(sprow_xpd(r2, 2 * r2->maxlen + 1, TYPE_SPMAT), "bkp_swap_elt");
    }
...
#ifndef ANSI_C
row_elt *bkp_bump_col(A, j, row, idx) SPMAT *A;
int j, *row, *idx;
#else
...
#ifndef ANSI_C
SPMAT *bkp_interchange(A, i1, i2) SPMAT *A;
int i1, i2;
#else
...
#endif
{
  ...
  if (idx1 >= 0 || idx2 >= 0) {
    tracecatch(bkp_swap_elt(A, i1, i1, idx1, i2, i2, idx2), "bkp_interchange");
  }
...
#ifndef ANSI_C
static double max_row_col(A, i, j, l) SPMAT *A;
int i, j, l;
#else
...
#ifndef ANSI_C
static int col_cmp(e1, e2) row_elt *e1, *e2;
#else
...
#ifndef ANSI_C
SPMAT *spBKPfactor(A, pivot, blocks, tol) SPMAT *A;
PERM *pivot, *blocks;
double tol;
#else
...
#endif
{
  ...
  int row_num;
  int best_deg, best_j, best_l, best_cost, mark_cost, deg, deg_j, deg_l,
      ignore_deg;
  ...
  for (i = 0; i < n - 1; i = onebyone ? i + 1 : i + 2) {
    ...
    for (deg = 0; deg <= n; deg++) {
      ...
      while (list_idx < deg_list->dim && deg_list->ive[list_idx] <= deg) {
  ...
  dopivot:
    if (onebyone) {
      ...
      for (s_idx = 0; s_idx < scan_row->dim; s_idx++) {
        ...
        for (s_idx2 = s_idx; s_idx2 < scan_row->dim; s_idx2++) {
          ...
          if (scan_row->ive[s_idx2] ==
              j) { /* no fill-in -- can be done directly */
    ...
    {      /* do 2 x 2 pivot */
      ...
      for (s_idx = 0; s_idx < scan_row->dim; s_idx++) {
        ...
        if (j < i + 2)
          continue;
        tracecatch(scan_to(A, scan_row, scan_idx, col_list, j), "spBKPfactor");
        ...
        while (s_idx2 < scan_row->dim) {
          ...
          idx_k = orig_idx->ive[s_idx2];
          idx1_k = orig1_idx->ive[s_idx2];
          e_ik = (idx_k < 0) ? (row_elt *)NULL : &(r_piv->elt[idx_k]);
          e_ip1k = (idx1_k < 0) ? (row_elt *)NULL : &(r1_piv->elt[idx1_k]);
        ...
        if (idx >= 0) {
          ...
          if (r_piv->len >= r_piv->maxlen) {
            tracecatch(sprow_xpd(r_piv, 2 * r_piv->maxlen + 1, TYPE_SPMAT),
                       "spBKPfactor");
          }
        ...
        if (idx1 >= 0) {
          ...
          old_row = -1;
          old_idx = j;
          tracecatch(chase_col(A, j, &old_row, &old_idx, i), "spBKPfactor");
          /* sprow_set_val(r1_piv,j,t); */
          idx1 = r1_piv->len;
          if (r1_piv->len >= r1_piv->maxlen) {
            tracecatch(sprow_xpd(r1_piv, 2 * r1_piv->maxlen + 1, TYPE_SPMAT),
                       "spBKPfactor");
          }
...
#ifndef ANSI_C
VEC *spBKPsolve(A, pivot, block, b, x) SPMAT *A;
PERM *pivot, *block;
VEC *b, *x;
#else
VEC *spBKPsolve(SPMAT *A, PERM *pivot, PERM *block, const VEC *b, VEC *x)
#endif
{
