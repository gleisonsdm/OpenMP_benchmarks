...
/* rk4 -- 4th order Runge-Kutta method */
/* another variant */
double rk4_var(f, t, x, h) double t, h;
VEC *(*f)(), *x;
{
  ...
  /* register workspace variables */
  mem_stat_reg_vars(0, TYPE_VEC, __FILE__, __LINE__, &v1, &v2, &v3, &v4, &temp,
                    NULL);
  /* end of memory allocation */
...
void main() {
  ...
  input("Choose the problem (1=Runge-Kutta, 2=least squares,3=laplace): ", "%d",
        &i);
  switch (i) {
  case 1:
    tutor_rk4();
