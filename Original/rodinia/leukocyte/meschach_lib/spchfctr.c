...
#ifndef ANSI_C
static double sprow_ip(row1, row2, lim) SPROW *row1, *row2;
int lim;
#else
...
#ifndef ANSI_C
static double sprow_sqr(row, lim) SPROW *row;
int lim;
#else
...
/* set_scan -- expand scan_row and scan_idx arrays
        -- return new length */
#ifndef ANSI_C
int set_scan(new_len) int new_len;
#else
...
#endif
{
  ...
  for (k = 0; k < m; k++) {
    ...
    for (;;) /* forever do... */
    {
      ...
      if (idx < 0) { /* fill-in */
        sp_set_val(A, minim, k, -sprow_ip(r_piv, r_op, k) / pivot);
        ...
        elt_op[idx].val = (elt_op[idx].val - sprow_ip(r_piv, r_op, k)) / pivot;
...
#endif
{
  ...
  for (k = 0; k < m; k++) {
    ...
    while (nxt_row >= 0 && nxt_idx >= 0) {
      ...
      elt_op = r_op->elt;
      elt_op[nxt_idx].val =
          (elt_op[nxt_idx].val - sprow_ip(r_piv, r_op, k)) / pivot;
