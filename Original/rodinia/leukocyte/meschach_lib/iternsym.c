...
#endif
{
  ...
  if (ip->x != VNULL) {
    ...
    if (ip->Bx) {
      ...
      v_sub(ip->b, v, r);      /* r = b-A*x */
  } else {                     /* ip->x == 0 */
    ip->x = v_get(ip->b->dim); /* x == 0 */
    ip->shared_x = FALSE;
    if (ip->Bx)
      (ip->Bx)(ip->B_par, ip->b, r); /* r = B*b */
    else
      v_copy(ip->b, r); /* r = b */
  }
...
*/
#ifndef ANSI_C
VEC *iter_spcgs(A, B, b, r0, tol, x, limit, steps) SPMAT *A, *B;
VEC *b, *r0, *x;
double tol;
int *steps, limit;
#else
VEC *iter_spcgs(SPMAT *A, SPMAT *B, VEC *b, VEC *r0, double tol, VEC *x,
                int limit, int *steps)
#endif
{
  ITER *ip;
...
#endif
{
  ...
  for (ip->steps = 0; ip->steps <= ip->limit; ip->steps++) {
    ...
    if (ip->info)
      ip->info(ip, nres, w, VNULL);
    if (ip->steps == 0)
      ip->init_res = nres;
    if (ip->stop_crit(ip, nres, w, VNULL))
      break;
  }
...
#ifndef ANSI_C
VEC *iter_splsqr(A, b, tol, x, limit, steps) SPMAT *A;
VEC *b, *x;
double tol;
int *steps, limit;
#else
...
*/
#ifndef ANSI_C
MAT *iter_arnoldi_iref(ip, h_rem, Q, H) ITER *ip;
Real *h_rem;
MAT *Q, *H;
#else
...
*/
#ifndef ANSI_C
MAT *iter_arnoldi(ip, h_rem, Q, H) ITER *ip;
Real *h_rem;
MAT *Q, *H;
#else
...
#ifndef ANSI_C
MAT *iter_sparnoldi(A, x0, m, h_rem, Q, H) SPMAT *A;
VEC *x0;
int m;
Real *h_rem;
MAT *Q, *H;
...
/* for testing gmres */
#ifndef ANSI_C
static void test_gmres(ip, i, Q, R, givc, givs, h_val) ITER *ip;
int i;
MAT *Q, *R;
VEC *givc, *givs;
double h_val;
#else
static void test_gmres(ITER *ip, int i, MAT *Q, MAT *R, VEC *givc, VEC *givs,
                       double h_val)
#endif
{
  VEC vt, vt1;
...
#ifndef ANSI_C
VEC *iter_spgmres(A, B, b, tol, x, k, limit, steps) SPMAT *A, *B;
VEC *b, *x;
double tol;
int *steps, k, limit;
#else
...
/* for testing mgcr */
#ifndef ANSI_C
static void test_mgcr(ip, i, Q, R) ITER *ip;
int i;
MAT *Q, *R;
#else
...
#endif
{
  ...
  for (ip->steps = 0; ip->steps < ip->limit;) {
    ...
    for (i = 0; i < ip->k && ip->steps < ip->limit; i++) {
      ...
      if (fabs(nres - 1.0) <= MACHEPS * ip->init_res)
        ...
        if (nres < 0.0) {
          nres = sqrt((double)-nres);
          if (ip->info)
            (*ip->info)(ip, -dd *nres, VNULL, VNULL);
          break;
        }
        dd *= sqrt((double)nres);
      }
    ...
    if (done)
      break; /* stop the iterative process */
    alpha = v_resize(alpha, ip->k);
    H = m_resize(H, ip->k, ip->k);
...
/* no preconditioner */
#ifndef ANSI_C
VEC *iter_spmgcr(A, B, b, tol, x, k, limit, steps) SPMAT *A, *B;
VEC *b, *x;
double tol;
int *steps, k, limit;
#else
...
#endif
{
  ...
  for (ip->steps = 0; ip->steps <= ip->limit; ip->steps++) {
    ...
    nres = sqrt(fabs(inner));
    if (ip->info)
      ip->info(ip, nres, r, rr1);
    if (ip->steps == 0)
      ip->init_res = nres;
    if (ip->stop_crit(ip, nres, r, rr1))
      break;
...
*/
#ifndef ANSI_C
VEC *iter_spcgne(A, B, b, eps, x, limit, steps) SPMAT *A, *B;
VEC *b, *x;
double eps;
int *steps, limit;
#else
VEC *iter_spcgne(SPMAT *A, SPMAT *B, VEC *b, double eps, VEC *x, int limit,
                 int *steps)
#endif
{
  ITER *ip;
