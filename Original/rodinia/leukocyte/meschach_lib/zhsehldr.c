...
ZVEC *zhhvec(vec, i0, beta, out, newval) ZVEC *vec, *out;
int i0;
Real *beta;
complex *newval;
{
...
ZVEC *zhhtrvec(hh, beta, i0, in, out) ZVEC *hh, *in,
...
double beta;
{
  ...
  out = zv_copy(in, out);
  __zmltadd__(&(out->ve[i0]), &(hh->ve[i0]), scale, (int)(in->dim - i0),
              Z_NOCONJ);
  /************************************************************
  for ( i=i0; i<in->dim; i++ )
...
ZMAT *zhhtrrows(M, i0, j0, hh, beta) ZMAT *M;
int i0, j0;
ZVEC *hh;
double beta;
{
  ...
  for (i = i0; i < M->m; i++) {/* compute inner product */
    ip = __zip__(&(M->me[i][j0]), &(hh->ve[j0]), (int)(M->n - j0), Z_NOCONJ);
    /**************************************************
    ip = 0.0;
    for ( j = j0; j < M->n; j++ )
            ip += M->me[i][j]*hh->ve[j];
    **************************************************/
    ...
    /* do operation */
    __zmltadd__(&(M->me[i][j0]), &(hh->ve[j0]), scale, (int)(M->n - j0),
                Z_CONJ);
    /**************************************************
    for ( j = j0; j < M->n; j++ )
...
*/
ZMAT *zhhtrcols(M, i0, j0, hh, beta) ZMAT *M;
int i0, j0;
ZVEC *hh;
double beta;
{
...
ZMAT *_zhhtrcols(M, i0, j0, hh, beta, w) ZMAT *M;
int i0, j0;
ZVEC *hh;
double beta;
ZVEC *w;
{
  ...
  for (i = i0; i < M->m; i++)
    /* if ( hh->ve[i] != 0.0 ) */
    if (!is_zero(hh->ve[i]))
      __zmltadd__(&(w->ve[j0]), &(M->me[i][j0]), hh->ve[i], (int)(M->n - j0),
                  Z_CONJ);
  for (i = i0; i < M->m; i++)
    /* if ( hh->ve[i] != 0.0 ) */
    if (!is_zero(hh->ve[i])) {
      ...
      __zmltadd__(&(M->me[i][j0]), &(w->ve[j0]), scale, (int)(M->n - j0),
                  Z_CONJ);
    }
