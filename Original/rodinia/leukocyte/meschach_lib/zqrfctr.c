...
/* zQRCPfactor -- forms the QR factorisation of A with column pivoting
   -- factorisation stored in compact form as described above
   ( not quite standard format )				*/
ZMAT *zQRCPfactor(A, diag, px) ZMAT *A;
ZVEC *diag;
PERM *px;
{
...
/* zQsolve -- solves Qx = b, Q is an orthogonal matrix stored in compact
        form a la QRfactor()
        -- may be in-situ */
ZVEC *_zQsolve(QR, diag, b, x, tmp) ZMAT *QR;
ZVEC *diag, *b, *x, *tmp;
{
...
/* zmakeQ -- constructs orthogonal matrix from Householder vectors stored in
   compact QR form */
ZMAT *zmakeQ(QR, diag, Qout) ZMAT *QR, *Qout;
ZVEC *diag;
{
...
ZMAT *zmakeR(QR, Rout) ZMAT *QR, *Rout;
{
  unsigned int i, j;
...
ZVEC *zQRsolve(QR, diag, b, x) ZMAT *QR;
ZVEC *diag, *b, *x;
{
  int limit;
...
/* zQRAsolve -- solves the system (Q.R)*.x = b
        ...
        -- returns x, which is created if necessary */
ZVEC *zQRAsolve(QR, diag, b, x) ZMAT *QR;
ZVEC *diag, *b, *x;
{
  int j, limit;
  Real beta, r_ii, tmp_val;
...
ZVEC *zQRCPsolve(QR, diag, pivot, b, x) ZMAT *QR;
ZVEC *diag;
PERM *pivot;
ZVEC *b, *x;
{
