...
#ifndef ANSI_C
void iter_std_info(ip, nres, res, Bres) ITER *ip;
double nres;
VEC *res, *Bres;
#else
...
#ifndef ANSI_C
int iter_std_stop_crit(ip, nres, res, Bres) ITER *ip;
double nres;
VEC *res, *Bres;
#else
...
#ifndef ANSI_C
ITER *iter_get(lenb, lenx) int lenb, lenx;
#else
...
#endif
{
  ...
  if (!ip->shared_x && ip->x != NULL)
    v_free(ip->x);
  if (!ip->shared_b && ip->b != NULL)
    v_free(ip->b);
...
#ifndef ANSI_C
ITER *iter_resize(ip, new_lenb, new_lenx) ITER *ip;
...
*/
#ifndef ANSI_C
ITER *iter_copy2(ip1, ip2) ITER *ip1, *ip2;
#else
...
#ifndef ANSI_C
ITER *iter_copy(ip1, ip2) ITER *ip1, *ip2;
#else
...
/* iter_gen_sym -- generate symmetric positive definite
   ...
   */
#ifndef ANSI_C
SPMAT *iter_gen_sym(n, nrow) int n, nrow;
#else
...
*/
#ifndef ANSI_C
SPMAT *iter_gen_nonsym(m, n, nrow, diag) int m, n, nrow;
double diag;
#else
...
*/
#ifndef ANSI_C
SPMAT *iter_gen_nonsym_posdef(n, nrow) int n, nrow;
#else
SPMAT *iter_gen_nonsym(int m, int n, int nrow, double diag)
#endif
{
