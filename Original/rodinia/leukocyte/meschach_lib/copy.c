...
#ifndef ANSI_C
MAT *_m_copy(in, out, i0, j0) MAT *in, *out;
unsigned int i0, j0;
#else
...
#ifndef ANSI_C
VEC *_v_copy(in, out, i0) VEC *in, *out;
unsigned int i0;
#else
...
#ifndef ANSI_C
PERM *px_copy(in, out) PERM *in, *out;
#else
...
#ifndef ANSI_C
MAT *m_move(in, i0, j0, m0, n0, out, i1, j1) MAT *in, *out;
int i0, j0, m0, n0, i1, j1;
#else
MAT *m_move(const MAT *in, int i0, int j0, int m0, int n0, MAT *out, int i1,
            int j1)
#endif
{
  int i;
  ...
  for (i = 0; i < m0; i++)
    MEM_COPY(&(in->me[i0 + i][j0]), &(out->me[i1 + i][j1]), n0 * sizeof(Real));
...
#ifndef ANSI_C
VEC *v_move(in, i0, dim0, out, i1) VEC *in, *out;
int i0, dim0, i1;
#else
...
/* mv_move -- copies selected piece of matrix to a vector
        ...
        -- out is resized if necessary */
#ifndef ANSI_C
VEC *mv_move(in, i0, j0, m0, n0, out, i1) MAT *in;
VEC *out;
int i0, j0, m0, n0, i1;
#else
...
-- moves the subvector with initial index i0 and length m1*n1 to
        ...
        -- out is resized if necessary */
#ifndef ANSI_C
MAT *vm_move(in, i0, out, i1, j1, m1, n1) VEC *in;
MAT *out;
int i0, i1, j1, m1, n1;
#else
