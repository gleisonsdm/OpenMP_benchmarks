...
/* names of types */
static char *mem_type_names[] = {"MAT", "BAND", "PERM", "VEC", "IVEC"
#ifdef SPARSE
                                 ,
                                 "ITER", "SPROW", "SPMAT"
#endif
...
static int (*mem_free_funcs[MEM_NUM_STD_TYPES])() = {
    m_free, bd_free, px_free, v_free, iv_free
#ifdef SPARSE
    ,
    iter_free, sprow_free, sp_free
#endif
#ifdef COMPLEX
    ,
    zv_free, zm_free
...
MEM_CONNECT mem_connect[MEM_CONNECT_MAX_LISTS] = {
    {mem_type_names, mem_free_funcs, MEM_NUM_STD_TYPES, mem_info_sum}};

/* attach a new list of types */
#ifndef ANSI_C
int mem_attach_list(list, ntypes, type_names, free_funcs, info_sum) int list,
    ntypes;            /* number of a list and number of types there */
...
#endif
{
  ...
  if (type_names == NULL || free_funcs == NULL || info_sum == NULL ||
      ntypes < 0)
    return -1;
...
#ifndef ANSI_C
int mem_free_vars(list) int list;
#else
...
#ifndef ANSI_C
int mem_is_list_attached(list) int list;
#else
...
#ifndef ANSI_C
void mem_dump_list(fp, list) FILE *fp;
int list;
#else
...
#endif
{
  ...
  fprintf(fp, " %-7s   %-12s   %-9s   %s\n", "name of", "alloc.", "# alloc.",
          "address");
  fprintf(fp, " %-7s   %-12s   %-9s   %s\n", " type", "bytes", "variables",
          "of *_free()");

  for (i = 0; i < mlist->ntypes; i++)
    fprintf(fp, "  %-7s   %-12ld   %-9d   %p\n", mlist->type_names[i],
            mlist->info_sum[i].bytes, mlist->info_sum[i].numvar,
            mlist->free_funcs[i]);
...
#ifndef ANSI_C
long mem_info_bytes(type, list) int type, list;
#else
...
#endif
{
  if (list < 0 || list >= MEM_CONNECT_MAX_LISTS)
    return 0l;
  if (!mem_switched_on || type < 0 || type >= mem_connect[list].ntypes ||
...
#ifndef ANSI_C
int mem_info_numvar(type, list) int type, list;
#else
...
#ifndef ANSI_C
void mem_info_file(fp, list) FILE *fp;
int list;
#else
...
#endif
{
  ...
  for (type = 0; type < mlist->ntypes; type++) {
    ...
    fprintf(fp, " type %-7s %10ld alloc. byte%c  %6d alloc. variable%c\n",
            mlist->type_names[type], d, (d != 1 ? 's' : ' '), n,
            (n != 1 ? 's' : ' '));
  }
...
/* mem_bytes_list
   ...
   */
#ifndef ANSI_C
void mem_bytes_list(type, old_size, new_size, list) int type, list;
...
#endif
{
  ...
  mlist = &mem_connect[list];
  if (type < 0 || type >= mlist->ntypes || mlist->free_funcs[type] == NULL)
    return;
...
#ifndef ANSI_C
void mem_numvar_list(type, num, list) int type, list, num;
#else
...
#ifndef MEX
      fprintf(stderr, "\n WARNING !! memory info: allocated # of variables is "
                      "less than 0\n");
      fprintf(stderr, "\t TYPE %s \n\n", mlist->type_names[type]);
      if (!isatty(fileno(stdout))) {
        fprintf(stdout, "\n WARNING !! memory info: allocated # of variables "
                        "is less than 0\n");
        fprintf(stdout, "\t TYPE %s \n\n", mlist->type_names[type]);
      }
#else
