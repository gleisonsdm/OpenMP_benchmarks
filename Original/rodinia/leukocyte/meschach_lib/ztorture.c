...
int cmp_perm(pi1, pi2) PERM *pi1, *pi2;
{
  int i;
...
char *argv[];
{
  ...
  ZMAT *A = ZMNULL, *B = ZMNULL, *C = ZMNULL, *D = ZMNULL, *Q = ZMNULL;
  complex ONE;
  complex z1, z2, z3;
  ...
  if (fabs(z1.re * z2.re - z1.im * z2.im - z3.re) +
          fabs(z1.im * z2.re + z1.re * z2.im - z3.im) >
      10 * MACHEPS)
    errmesg("zmlt");
  s1 = zabs(z1);
  ...
  catch (E_NULL, catchall(zv_add(ZVNULL, ZVNULL, ZVNULL);
                          errmesg("tracecatch() failure"),
                          printf("# tracecatch() caught error\n");
                          error(E_NULL, "main"));
         errmesg("catch() failure"), printf("# catch() caught E_NULL error\n"));
  ...
  ******************************/
  ...
  if ((fp = fopen(SAVE_FILE, "w")) == (FILE *)NULL)
    ...
    if ((fp = fopen(SAVE_FILE, "r")) == (FILE *)NULL)
      ...
      ZM_FREE(B);
      B = zm_load(fp, &cp);
      if (strcmp(name, cp) || zm_norm1(zm_sub(A, B, C)) >= MACHEPS * A->m) {
