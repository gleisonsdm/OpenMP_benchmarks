...
#ifndef ANSI_C
ZMAT *izm_finput(fp, mat) FILE *fp;
ZMAT *mat;
#else
  ...
  redo:
    ...
    for (j = 0; j < n; j++)
      ...
      redo2:
        ...
        if (!dynamic)
          fprintf(stderr, "old (%14.9g,%14.9g) new: ", mat->me[i][j].re,
                  mat->me[i][j].im);
        if (fgets(line, MAXLINE, fp) == NULL)
          error(E_INPUT, "izm_finput");
...
#ifndef ANSI_C
ZMAT *bzm_finput(fp, mat) FILE *fp;
ZMAT *mat;
#else
...
#ifndef ANSI_C
ZVEC *izv_finput(fp, vec) FILE *fp;
ZVEC *vec;
#else
...
#endif
{
  ...
  for (i = 0; i < dim; i++)
    ...
    redo:
      ...
      if (!dynamic)
        fprintf(stderr, "old (%14.9g,%14.9g) new: ", vec->ve[i].re,
                vec->ve[i].im);
      if (fgets(line, MAXLINE, fp) == NULL)
        error(E_INPUT, "izv_finput");
...
#ifndef ANSI_C
ZVEC *bzv_finput(fp, vec) FILE *fp;
ZVEC *vec;
#else
...
#ifndef ANSI_C
char *setzformat(f_string) char *f_string;
#else
