...
#endif
{
  ...
  if (mat == (MAT *)NULL || (int)(mat->m) < 0 || (int)(mat->n) < 0)
    /* don't trust it */
    return (-1);
...
#endif
{
  ...
  if (px->pe == (unsigned int *)NULL) {
    ...
    if (mem_info_is_on()) {
      mem_bytes(TYPE_PERM, sizeof(PERM) + px->max_size * sizeof(unsigned int),
                0);
      mem_numvar(TYPE_PERM, -1);
    }
    free((char *)px->pe);
...
#ifndef ANSI_C
MAT *m_resize(A, new_m, new_n) MAT *A;
int new_m, new_n;
#else
...
#endif
{
  ...
  if (new_m > A->max_m) { /* re-allocate A->me */
    if (mem_info_is_on()) {
      mem_bytes(TYPE_MAT, A->max_m * sizeof(Real *), new_m * sizeof(Real *));
    }
  ...
  if (old_n > new_n) {
    for (i = 1; i < min(old_m, new_m); i++)
      MEM_COPY((char *)&(A->base[i * old_n]), (char *)&(A->base[i * new_n]),
               sizeof(Real) * new_n);
  } else if (old_n < new_n) {
    for (i = (int)(min(old_m, new_m)) - 1; i > 0;
         i--) { /* copy & then zero extra space */
      MEM_COPY((char *)&(A->base[i * old_n]), (char *)&(A->base[i * new_n]),
               sizeof(Real) * old_n);
  ...
  if (A->max_n < new_n) {
    ...
    for (i = 0; i < A->max_m; i++) {
      if (mem_info_is_on()) {
        mem_bytes(TYPE_MAT, A->max_n * sizeof(Real), new_max_n * sizeof(Real));
      }
...
#ifndef ANSI_C
PERM *px_resize(px, new_size) PERM *px;
int new_size;
#else
...
#endif
{
  ...
  if (new_dim > x->max_dim) {
    if (mem_info_is_on()) {
      mem_bytes(TYPE_VEC, x->max_dim * sizeof(Real), new_dim * sizeof(Real));
    }
