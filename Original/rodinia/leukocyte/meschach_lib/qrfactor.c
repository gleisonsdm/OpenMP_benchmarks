...
#ifndef ANSI_C
MAT *QRCPfactor(A, diag, px) MAT *A;
VEC *diag;
PERM *px;
#else
...
#ifndef ANSI_C
VEC *_Qsolve(QR, diag, b, x, tmp) MAT *QR;
VEC *diag, *b, *x, *tmp;
#else
...
/* makeQ -- constructs orthogonal matrix from Householder vectors stored in
   compact QR form */
#ifndef ANSI_C
MAT *makeQ(QR, diag, Qout) MAT *QR, *Qout;
VEC *diag;
#else
...
#ifndef ANSI_C
VEC *QRsolve(QR, diag, b, x) MAT *QR;
VEC *diag /* , *beta */, *b, *x;
#else
...
#ifndef ANSI_C
VEC *QRCPsolve(QR, diag, pivot, b, x) MAT *QR;
VEC *diag;
PERM *pivot;
VEC *b, *x;
#else
VEC *QRCPsolve(const MAT *QR, const VEC *diag, PERM *pivot, const VEC *b,
               VEC *x)
#endif
{
  STATIC VEC *tmp = VNULL;
...
#ifndef ANSI_C
static VEC *UTmlt(U, x, out) MAT *U;
VEC *x, *out;
#else
...
#ifndef ANSI_C
VEC *QRTsolve(A, diag, c, sc) MAT *A;
VEC *diag, *c, *sc;
#else
