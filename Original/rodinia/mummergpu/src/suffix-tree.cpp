...
public:
  ...
  void stop() { gettimeofday(&m_end, NULL); }

  /// Return the duration in seconds
  double duration() {
    if ((m_end.tv_sec == 0) && (m_end.tv_usec == 0)) {
      stop();
    }
    return ((m_end.tv_sec - m_start.tv_sec) * 1000000.0 +
...
public:
  ...
  SuffixNode(int s, int e, int leafid, SuffixNode *p, SuffixNode *x)
      : m_start(s), m_end(e), m_nodeid(++s_nodecount), m_leafid(leafid),
        m_numleaves(0), m_parent(p), m_suffix(x), m_printParent(NULL) {
...
public:
  ...
  void printNodeDot(SuffixNode *node, ostream &dfile) {
    ...
    if (node->m_suffix) {
      ...
      // node->m_suffix->printLabel(dfile, m_string) << " [style=dotted,
    ...
    if (children == 0) {
      ...
      dfile << " " << node << " [shape=box,width=.2,height=.2,label=\""
            << node->id() << ":" << node->m_leafid << "\"]" << endl;
    } else {
      // dfile << " " << node << " [label=";
      // node->printLabel(dfile, m_string) << "]" << endl;
      dfile << " " << node << " [width=.2,height=.2,label=\"" << node->id()
            << "\"]" << endl;
    }
  }
  ...
  void printDot(const char *dotfilename) {
    ...
    dfile << " label=\"Suffix tree of \'" << m_string
          << "\' len:" << m_strlen - 1 << " nc:" << SuffixNode::s_nodecount
          << "\"" << endl;
  ...
  void printTreeSorted(ostream &out, SuffixNode *node,
                       const string &pathstring) {
    bool isLeaf = true;

    string ps(pathstring);
    if (node != m_root) {
      ps.append(node->str(m_string));
    }
  ...
  void printNodeFlat(ostream &out, SuffixNode *node) {
    out << node->id() << "\t" << node->m_parent->id() << "\t"
        << node->m_suffix->id() << "\t" << node->m_start << "\t" << node->m_end
        << "\t";
    ...
    if (node == m_root) {
      ...
      out << node->str(m_string) << endl;
    }

    for (int i = 0; i < basecount; i++) {
      if (node->m_children[i]) {
        printNodeFlat(out, node->m_children[i]);
      }
    }
  }
  ...
  void setNodePath(SuffixNode *node, const string &parentString) {
    ...
    if (node != m_root) {
      node->m_pathstring.append(m_string, node->m_start,
                                node->m_end - node->m_start + 1);
    }
  ...
  void buildUkkonen() {
    int len =
        m_strlen - 1; // length of the string, not of the buffer (remove s)
...
// Construct T1
#ifdef MPOOL
    SuffixNode *node =
        new (&m_pool) SuffixNode(1, len, 1, m_root, NULL); // leaf: 1
#else
    ...
    for (int i = 2; i <= len; i++) {
      ...
      // Keep track of last internal nodes created in split so we can add suffix
      // links
      ...
      if (DEBUG) {
        ...
        cerr << endl;
        cerr << i << ".0 "
             << "Phase " << i << " adding " << next << " starting with "
             << startj << endl;

        string beta = substr(m_string, 1, i);
        cerr << i << ".1"
             << " Extension 1: \"" << beta << "\" [implicit]" << endl;
      }
      ...
      for (int j = startj; j <= i; j++) {
        ...
        //       2) Walk up the tree w bases until we get to a node with a
        //       suffix link.
        //       3) Follow suffix link which shifts the path from S[j-1..i] to
        //       S[j..i]
        //       4) Walk down tree in new location ensuring S[i-w .. i] is in
        //       tree
        ...
        int betapos =
            i; // The first position in string we need to check in tree

        if (DEBUG) {
          cerr << endl;
          string beta = substr(m_string, j, i - j + 1);
          cerr << i << "." << j << " Phase " << i << " Extension " << j
               << ": \"" << beta << "\" bp:" << betapos << endl;
        ...
        if (node == m_root) {
          ...
          if (node->m_suffix == NULL) {
            ...
            if (DEBUG) {
              cerr << i << "." << j << "   grandparent bp: " << betapos
                   << " n:";
              node->printLabel(cerr, m_string) << endl;
            }
        ...
        if (node == m_root) {
          betapos = j;
        } // have to check full string

        if (DEBUG) {
          cerr << i << "." << j
               << "  Starting to walk down from bp: " << betapos << " to " << i
               << " n:";
          node->printLabel(cerr, m_string) << endl;
        }
        ...
        bool done = false;
        startj =
            j + 1; // assume this extension should be skipped in the next phase
        ...
        while ((betapos <= i) && !done) {
          ...
          if (DEBUG) {
            cerr << i << "." << j << "  node betapos: " << betapos << "["
                 << base << "] n:";
            node->printLabel(cerr, m_string) << " ";
            if (child) {
              cerr << "c: ";
              child->printLabel(cerr, m_string);
            }
            cerr << endl;
          }
          ...
          if (!child) {
...
#ifdef MPOOL
            SuffixNode *newnode = new (&m_pool)
                SuffixNode(betapos, len, j, node, NULL); // leaf: j
#else
            SuffixNode *newnode =
                new SuffixNode(betapos, len, j, node, NULL); // leaf: j
#endif
            ...
            if (nodebase != base) {
              ...
              cerr << "ERROR: first base on edge doesn't match edge label"
                   << endl;
            ...
            if (DOJUMP) {
              ...
              if (mustmatch >= childlen) {
                ...
                if (DEBUG) {
                  cerr << i << "." << j << "   Edge Jump by: " << childlen
                       << " new bp: " << betapos << " np: " << nodepos << endl;
                }

#if VERIFY
                if (nodepos != child->m_end + 1) {
                  cerr << "ERROR: jump should have skipped entire edge, but "
                          "didn't!"
                       << endl;
                  exit(1);
                }
                ...
                if (DEBUG) {
                  cerr << i << "." << j << "   Partial Jump by: " << mustmatch
                       << " new bp: " << betapos << " np: " << nodepos << endl;
                }
                ...
                if (VERIFY) {
                  if (m_string[betapos - 1] != m_string[nodepos - 1]) {
                    cerr << "ERROR: jump should have matched at least the "
                            "mustmatch-1 characters"
                         << endl;
                    cerr << "s[bp-1]: " << m_string[betapos - 1]
                         << " s[np-1]: " << m_string[nodepos - 1] << endl;
                    exit(1);
                  }
                }
#endif
            ...
            while (nodepos <= child->m_end && betapos <= i) {
...
#if VERBOSE
              cerr << i << "." << j << "   child bp: " << betapos << "["
                   << m_string[betapos] << "] nb [" << nodebase << "]" << endl;
#endif
              ...
              if (m_string[betapos] == nodebase) {
                ...
                if (betapos == i + 1) {
                  if (DEBUG) {
                    cerr << i << "." << j
                         << "    Internal edge match nw: " << nodewalk << endl;
                  }

                  if ((nodewalk == child->len(i)) && (child->m_end == len)) {
                    // we walked the whole edge to leaf, implicit rule I
                    // extension
                    if (DEBUG) {
                      cerr << i << "." << j
                           << "    Leaf Node, Implicit Rule I Extension"
                           << endl;
                    }
                  } else {
                    // "Real" rule III implicit extension

                    // The j-1 extension was the last explicit extension in this
                    // round
                    ...
                    if (DOPHASETRICK) {
                      ...
                      if (DEBUG) {
                        cerr << i << "." << j
                             << "    Implicit Extension... start next phase at "
                             << startj << ", saved " << skip << endl;
                      }
                    ...
                    if (DOINTERNALSKIP) {
                      // Since we hit an internal match on a non-leaf, we know
                      // every other
                      // extension in this phase will also hit an internal
                      // match.

                      // Have to be careful since leafs get the full string
                      // immediately, but
                      ...
                      if (DEBUG) {
                        cerr << i << "." << j
                             << "    Implicit Extension... skipping rest of "
                                "phase, saved "
                             << skip << endl;
                      }
...
#ifdef MPOOL
                SuffixNode *split = new (&m_pool) SuffixNode(
                    child->m_start, nodepos - 1, 0, node, NULL); // internal
#else
                SuffixNode *split = new SuffixNode(child->m_start, nodepos - 1,
                                                   0, node, NULL); // internal
#endif
...
#ifdef MPOOL
                SuffixNode *newnode = new (&m_pool)
                    SuffixNode(betapos, len, j, split, NULL); // leaf j
#else
                SuffixNode *newnode =
                    new SuffixNode(betapos, len, j, split, NULL); // leaf j
#endif
                lastleaf = newnode;
...
void buildNodeTexture(SuffixNode *node, PixelOfNode *nodeTexture,
  ...
  if (node->m_leafid != 0) {
    ...
    if (node->m_children[0]) {
      ...
      buildNodeTexture(node->m_children[0], nodeTexture, childrenTexture,
                       aux_data, refstr);
    }
    ...
    if (node->m_children[1]) {
      ...
      buildNodeTexture(node->m_children[1], nodeTexture, childrenTexture,
                       aux_data, refstr);
    }
    ...
    if (node->m_children[2]) {
      ...
      buildNodeTexture(node->m_children[2], nodeTexture, childrenTexture,
                       aux_data, refstr);
    }
    ...
    if (node->m_children[3]) {
      ...
      buildNodeTexture(node->m_children[3], nodeTexture, childrenTexture,
                       aux_data, refstr);
    }
    ...
    if (node->m_children[4]) {
      ...
      buildNodeTexture(node->m_children[4], nodeTexture, childrenTexture,
                       aux_data, refstr);
    }
  }
}
...
void buildSuffixTreeTexture(PixelOfNode **nodeTexture,
                            PixelOfChildren **childrenTexture,
                            unsigned int *width, unsigned int *node_height,
  ...
  *width = MAX_TEXTURE_DIMENSION;
  ...
  int numrows = ((int)ceil((allnodes + 0.0) /
                           ((MAX_TEXTURE_DIMENSION / 2) * TEXBLOCKSIZE))) *
                TEXBLOCKSIZE;
  ...
  *width = MAX_TEXTURE_DIMENSION;
  int numrows =
      ((int)ceil((allnodes + 0.0) / ((MAX_TEXTURE_DIMENSION)*TEXBLOCKSIZE))) *
      TEXBLOCKSIZE;
  *node_height = numrows;
  ...
  *nodeTexture =
      (PixelOfNode *)calloc((*width) * (*node_height), sizeof(PixelOfNode));
  alloc += (*width) * (*node_height) * sizeof(PixelOfNode);

  *childrenTexture = NULL;

  if (*children_height) {
    *childrenTexture = (PixelOfChildren *)calloc((*width) * (*children_height),
  ...
  *aux_data = (AuxiliaryNodeData *)calloc(allnodes, sizeof(AuxiliaryNodeData));
  ...
  buildNodeTexture(gtree->m_root, *nodeTexture, *childrenTexture, *aux_data,
                   refstr);
};

void printTreeTexture(const char *texfilename, PixelOfNode *nodeTexture,
                      PixelOfChildren *childrenTexture, int nodecount) {
  cerr << "Printing tree texture to " << texfilename << endl;

  ofstream texfile;
  texfile.open(texfilename, ofstream::out | ofstream::trunc);

  texfile << "id\tx\ty\tstart\tend\tdepth\ta.x\ta.y\tc.x\tc.y\tg.x\tg.y\tt."
             "x\tt.y\t$.x\t$.y\tp.x\tp.y"
          << endl;
  for (int i = 0; i < nodecount; i++) {
    TextureAddress myaddress(id2addr(i));

    texfile << i << "\t" << myaddress.x << "\t"
#if REORDER_TREE
            << myaddress.y << "\t"
#endif
            << MK3(nodeTexture[i].start) << "\t" << MK3(nodeTexture[i].end)
...
void renumberTree() {
  ...
  while (!nodequeue.empty()) {
    pair<SuffixNode *, int> npair = nodequeue.front();
    nodequeue.pop();
    ...
    if (depth < 16) {
      for (int i = 0; i < basecount; i++) {
        SuffixNode *child = node->m_children[i];
        if (child) {
          nodequeue.push(make_pair(child, depth + 1));
        }
      }
    } else {
      for (int i = 0; i < basecount; i++) {
                  ...
                  for (int l = 0; l < basecount; l++) {
                    ...
                    if (gggchild) {
                      ...
                      for (int m = 0; m < basecount; m++) {
                        ...
                        if (ggggchild) {
                          nodequeue.push(make_pair(ggggchild, depth + 5));
                        }
                      }
                    }
                  }
...

{
  ...
  cerr << SuffixNode::s_nodecount << " nodes " << btimer.str(true, 5) << endl;
  ...
  buildSuffixTreeTexture(nodeTexture, childrenTexture, width, node_height,
                         children_height, aux_data, gtree->m_string,
                         min_match_len);
  ...
  *num_nodes = SuffixNode::s_nodecount + 1;
  ...
  if (texfilename) {
    printTreeTexture(texfilename, *nodeTexture, *childrenTexture,
...
extern "C" void getReferenceString(const char *filename, char **refstr,
  ...
  if (refline[0] != '>') {
    cerr << endl << "ERROR: Reference file is not in FASTA format" << endl;
  }
  ...
  while (file >> buffer) {
    if (buffer[0] == '>') {
      cerr << endl
           << "ERROR: Only a single reference sequence is supported!" << endl;
...
extern "C" void
getQueriesTexture(int qfile, char **queryTexture, size_t *queryTextureSize,
                  int **queryAddrs, char ***queryNames, int **queryLengths,
                  unsigned int *numQueries, unsigned int *num_match_coords,
                  unsigned int memory_avail, int min_match_length, bool rc) {
  EventTime_t timer;
  ...
  while ((bytes_read = read(qfile, buf, sizeof(buf))) != 0) {
    ...
    for (; i < bytes_read; i++) {
      ...
      if (b == '>') {
        ...
        if (curr_mem_usage +
                bytesNeededOnGPU(MAX_QUERY_LEN, min_match_length) >=
            memory_avail) {
          set_full = true;
        ...
        if (offsetspos != 0) {
          if (this_qrylen < min_match_length) {
            ...
            // we really ought to pass the real length in, and then add the
            // padding
            // For now, we are overestimating.
            curr_mem_usage +=
                bytesNeededOnGPU(warp_max_qry_len, min_match_length);
  ...
  if (qstringpos) {
    if (this_qrylen < min_match_length) {
      ...
      if ((++num_warp_queries) % WARP_SIZE) {
        curr_warp_padding +=
            warp_max_qry_len * (WARP_SIZE - (num_warp_queries % WARP_SIZE));
      }
#else
      curr_mem_usage += bytesNeededOnGPU(this_qrylen, min_match_length);
#endif
  ...
  *queryLengths = lengths;
  ...
  for (unsigned int i = 0; i < *numQueries; i += WARP_SIZE) {
    ...
    unsigned int block_size =
        WARP_SIZE *
        (max_qry_len + ((max_qry_len % 4) ? 4 - (max_qry_len % 4) : 0));
    int *reordered_query_chars =
        (int *)calloc(block_size / sizeof(int), sizeof(int));
    // fprintf (stderr, "reordering block for warp %d\n", i / WARP_SIZE);

    for (int curr_char = 0; curr_char < max_qry_len; ++curr_char) {
      for (unsigned int curr_qry = 0;
           curr_qry < WARP_SIZE && (i + curr_qry) < *numQueries; ++curr_qry) {
        ...
        if (curr_char < qlen) {
          int *qry_word =
              reordered_query_chars + ((curr_char / 4) * WARP_SIZE + curr_qry);
          int c = *(qstring + offsets[i + curr_qry] + curr_char);
          //*((char*) ((void*)qry_word + (curr_char & 0x0000000f) = c;
          //*qry_word |= c << ((curr_char & 0x0000000f) << (3 - (curr_char &
          // 0x0000000f)));
  ...
  *num_match_coords = 0;
  for (unsigned int i = 0; i < reordered_query_chunks.size(); ++i) {
    for (unsigned int j = 0; j < WARP_SIZE && (i * WARP_SIZE + j) < *numQueries;
         ++j) {
      int qryid = i * WARP_SIZE + j;
      offsets[qryid] = (p / 4) + j;
    }
    //*num_match_coords += reordered_query_chunks[i].second - WARP_SIZE *
    //(min_match_length + 1);
    memcpy(*queryTexture + p, reordered_query_chunks[i].first,
           reordered_query_chunks[i].second);
  ...
  *num_match_coords = *queryTextureSize - *numQueries * (min_match_length + 1);
  ...
  cerr << offsetspos << " queries (" << qrylen << " bp), need "
       << curr_mem_usage << " bytes on the GPU (" << memory_avail << " avail) "
...
// FIXME: needs to be reinitialized to zero at the beginning of each round of
// printing.
...
#if REORDER_TREE
#define GETNODE(node_addr)                                                     \
  (((PixelOfNode *)(page->ref.h_node_tex_array)) + (node_addr.x) +             \
   (node_addr.y * MAX_TEXTURE_DIMENSION))
#define GETCHILDREN(node_addr)                                                 \
  (((PixelOfChildren *)(page->ref.h_children_tex_array)) + (node_addr.x) +     \
   (node_addr.y * MAX_TEXTURE_DIMENSION))
...
char *getMerged(const ReferencePage *page, TextureAddress cur,
                int getChildrenData) {
#if !REORDER_TREE
...
#define SDEPTH(node_addr)                                                      \
  (((PixelOfNode *)getMerged(page, node_addr, 0))->depth)
#define PARENT(node_addr)                                                      \
  (((PixelOfNode *)getMerged(page, node_addr, 0))->parent)

#define LEAFCHAR(node_addr)                                                    \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->leafchar)
#define LEAFID(node_addr)                                                      \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->leafid)
#define CHILDA(node_addr)                                                      \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->a)
#define CHILDC(node_addr)                                                      \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->c)
#define CHILDG(node_addr)                                                      \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->g)
#define CHILDT(node_addr)                                                      \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->t)
#define CHILDD(node_addr)                                                      \
  (((PixelOfChildren *)getMerged(page, node_addr, 1))->d)
...
void printNodeAlignments(const ReferencePage *page,
                         ...
                         const TextureAddress node, const int qrypos,
                         int qrylen, const pathblock path[], int path_idx,
                         bool rc) {
  ...
  if (isLeaf) {
    ...
    if (child.data) {
      printNodeAlignments(page, queryflankingbase, child, qrypos, qrylen, path,
                          path_idx, rc);
    }
...
// FIXME: hardcoded path buffer, needs to be as long as the longest query in the
// query set.
pathblock path[8192];
...
void printAlignmentsOld(ReferencePage *page, char *query, int qrylen,
                        int nodeid, int qrypos, int edge_match, int min_match,
                        bool rc, bool forwardcoordinates) {
  ...
  if (edge_match > 0) {
    string_depth =
        MK3(SDEPTH(node_addr)) - (NODE_LENGTH(nodeid) - edge_match) - 1;
  } else {
  ...
  if (rc) {
    ...
    if (forwardcoordinates) {
      qrypos = qrylen - 1 - qrypos;
    }
  }

  printNodeAlignments(page, flankingbase, prev, qrypos + 1, MK3(SDEPTH(prev)),
                      path, path_idx - 1, rc);
}
...
// Stackless printing
void printAlignments(ReferencePage *page, Alignment *h_alignments, char *query,
                     int qrylen, TextureAddress matchNodeAddr, int qrystartpos,
                     int edge_match, int min_match, bool rc,
  ...
  if (rc) {
    ...
    if (forwardcoordinates) {
      qrystartpos = qrylen - 1 - qrystartpos;
    }
  }
  ...
  if (verbose) {
    ...
    cout << "starting node: " << PADDR(cur) << " depth: " << MK3(SDEPTH(cur))
         << endl;
  }
  ...
  while (MK3(SDEPTH(cur)) > min_match) {
    ...
    if (verbose) {
      cout << "par: " << PADDR(cur) << " depth: " << MK3(SDEPTH(cur)) << endl;
    }
  }
  ...
  if (printParent.data != NODE_PRINTPARENT(matchNodeId).data) {
    ...
    cout << "printParent (" << PADDR(printParent)
         << "):" << MK3(SDEPTH(printParent)) << " != NODE_PRINTPARENT ("
         << PADDR(other) << "):" << MK3(SDEPTH(other)) << endl;
    exit(1);
  }
  ...
  // If the printparent is the matchnode, then we are already off the golden
  // path
  ...
  while (cur.data != badParent.data) {
    if (verbose) {
      cout << " cur: " << PADDR(cur) << " curchild: " << curchild << endl;
    }
    ...
    if (isLeaf || forceToParent) {
      ...
      if (isLeaf) {
        ...
        if (isLeaf != queryflankingbase) {
          ...
          if ((left_in_ref != page->begin || page->shadow_left == -1) &&
              (right_in_ref != page->end || page->shadow_right == -1)) {
            if (!(left_in_ref > page->begin &&
                  right_in_ref < page->shadow_left)) {
              leavesprinted++;
      ...
      if (cur.data == arrayToAddress(CHILDA(myParent)).data) {
        curchild = 'C';
      } else if (cur.data == arrayToAddress(CHILDC(myParent)).data) {
        curchild = 'G';
      } else if (cur.data == arrayToAddress(CHILDG(myParent)).data) {
        curchild = 'T';
      } else if (cur.data == arrayToAddress(CHILDT(myParent)).data) {
        curchild = '$';
        ...
        if (depthToGoldenPath == 0) {
          if (curchild == goldenChild) {
            ...
            if (cid == matchNodeId) {
              ...
              if (edge_match > 0) {
                matchlen =
                    MK3(SDEPTH(cur)) - (NODE_LENGTH(cid) - edge_match) - 1;
              }
            }
          } else {
            depthToGoldenPath = 1;
