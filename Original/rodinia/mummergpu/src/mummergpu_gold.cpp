...
#define MATCH_BASE(match_coords, qryid)                                        \
  (MatchCoord *) match_coords + qryAddr - (qryid * (min_match_len + 1))
...
#define GETNODE(addr, two_level)                                               \
  getMerged(nodes, childrenarr, addr, 0, NULL, NULL).node
#define GETNODEHIST(addr, two_level)                                           \
  getMerged(nodes, childrenarr, addr, 0, node_hist, child_hist).node
#define GETCHILDREN(addr, two_level)                                           \
  getMerged(nodes, childrenarr, addr, 1, NULL, NULL).children
#define GETCHILDRENHIST(addr, two_level)                                       \
  getMerged(nodes, childrenarr, addr, 1, node_hist, child_hist).children
#else
...
#define GETCHILDREN(addr, two_level)                                           \
  getMerged(nodes, childrenarr, addr, 1).children
#define GETCHILDRENHIST(addr, two_level)                                       \
  getMerged(nodes, childrenarr, addr, 1).children
#endif
...
#if TREE_ACCESS_HISTOGRAM
#define GETCHILDRENHIST(addr, two_level)                                       \
  getChildren(addr, childrenarr, child_hist)
...
char getRef(int refpos, char *ref) { return ref[refpos]; }
...
#if REORDER_TREE
  return *(nodes + (cur & 0x0000FFFF) +
           (((cur & 0xFFFF0000) >> 16) * MAX_TEXTURE_DIMENSION));
#else
  return *(nodes + cur);
#endif
}
...
#if REORDER_TREE
  return *(childrenarr + (cur & 0x0000FFFF) +
           (((cur & 0xFFFF0000) >> 16) * MAX_TEXTURE_DIMENSION));
#else
...
SingleNode getMerged(PixelOfNode *nodes, PixelOfChildren *childrenarr,
                     unsigned int cur, int getChildrenData
#if TREE_ACCESS_HISTOGRAM
                     ,
...
void set_result(unsigned int cur, MatchCoord *result, int edge_match_length,
                int qry_match_len, int min_match_len, int rc) {
...
#if REORDER_TREE
  addr = (arr[0] | ((arr[2] & 0xF) << 8)) |
         ((arr[1] | ((arr[2] & 0xF0) << 4)) << 16);
#else
  addr = MK3(arr);
#endif
}
...
int kernel_gold(int qryid, MatchResults *results, char *queries,
                PixelOfNode *nodes, PixelOfChildren *childrenarr, char *ref,
                const int *queryAddrs, const int *queryLengths,
                const int numQueries, const int min_match_len
#if TREE_ACCESS_HISTOGRAM
                ,
                int *node_hist, int *child_hist
#endif
  ...
  RECORD_RESULT : {
    ...
    SET_RESULT(cur, result, refpos - node_start, qry_match_len, min_match_len,
               FORWARD);
  ...
  NEXT_SUBSTRING : {
    ...
    // XPRINTF(" following suffix link. mustmatch:%d qry_match_len:%d
    // sl:("fNID")\n",
...
void rc_kernel_gold(int qryid, MatchResults *results, char *refstr,
                    char *queries, const int *queryAddrs,
                    const int *queryLengths, PixelOfNode *nodeTexture,
                    PixelOfChildren *childrenTexture, const int numQueries,
...
#pragma omp parallel for
    ...
    for (int i = 0; i < numQueries; ++i) {
      rc_kernel_gold(i, results, refstr, queries, queryAddrs, queryLengths,
                     nodeTexture, childrenTexture, numQueries, match_length);
    }
...
#pragma omp parallel for
    ...
    for (int i = 0; i < numQueries; ++i) {
      ...
      #endif
      */
      kernel_gold(i, results, queries, nodeTexture, childrenTexture, refstr,
                  queryAddrs, queryLengths, numQueries, match_length
#if TREE_ACCESS_HISTOGRAM
                  ,
                  int *node_hist, int *child_hist
#endif
