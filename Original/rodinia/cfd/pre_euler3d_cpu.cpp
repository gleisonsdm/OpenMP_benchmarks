...
#pragma omp parallel for default(shared) schedule(static)
  for (int i = 0; i < nelr; i++) {
    ...
    // dt = float(0.5f) * std::sqrt(areas[i]) /  (||v|| + c).... but when we do
...
void compute_flux_contributions(int nelr, float *variables,
  ...
  for (int i = 0; i < nelr; i++) {
    ...
    for (j = 0; j < NNB; j++) {
      ...
      } else if (nb == -2) // a far field boundary
      {
        ...
        flux_i_density_energy +=
            factor * (ff_fc_density_energy.x + fc_i_density_energy.x);
        ...
        flux_i_density_energy +=
            factor * (ff_fc_density_energy.y + fc_i_density_energy.y);
        ...
        flux_i_density_energy +=
            factor * (ff_fc_density_energy.z + fc_i_density_energy.z);
...
int main(int argc, char **argv) {
  ...
  // set far field conditions
  {
    ...
    compute_flux_contribution(ff_variable[VAR_DENSITY], ff_momentum,
                              ...
                              ff_velocity, ff_fc_momentum_x, ff_fc_momentum_y,
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    for (int j = 0; j < RK; j++) {
      compute_flux_contributions(nelr, variables, fc_momentum_x, fc_momentum_y,
                                 fc_momentum_z, fc_density_energy);
