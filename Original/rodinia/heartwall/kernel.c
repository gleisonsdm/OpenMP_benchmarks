...
void kernel(public_struct public, private_struct private) {
  ...
  private
    .d_tRowLoc[pointer] = private.d_Row[private.point_no];
  private
    .d_tColLoc[pointer] = private.d_Col[private.point_no];
  ...
  if (public.frame_no != 0) {
    ...
    for (col = 0; col < public.in2_cols; col++) {
      ...
      private
        .d_in2[col * public.in2_rows + row] = temp;
      private
        .d_in2_sqr[col * public.in2_rows + row] = temp * temp;
      }
    }
    ...
    for (col = 0; col < public.in_mod_cols; col++) {
      for (row = 0; row < public.in_mod_rows; row++) {
        ...
        temp = d_in[pointer];
      private
        .d_in_mod[col * public.in_mod_rows + row] = temp;
      private
      ...
      for (row = 1; row <= public.conv_rows; row++) {
        ...
        for (ja = ja1; ja <= ja2; ja++) {
          ...
          for (ia = ia1; ia <= ia2; ia++) {
            ib = ip1 - ia;
            s = s +
                private.d_in_mod[public.in_mod_rows * (ja - 1) + ia - 1] *
    ...
    //==================================================
    //	1) GET VERTICAL CUMULATIVE SUM SAVE
    // IN
    // d_in2_pad
    //==================================================
    ...
    //	3) SUBTRACT THE TWO SELECTIONS SAVE
    // IN
    // d_in2_sub
    //==================================================
    ...
    //	4) GET CUMULATIVE SUM 1 SQUARED SAVE
    // IN
    // d_in2_sub2_sqr
    //	5) GET NUMERATOR
    // SAVE
    ...
    for (col = 0; col < public.in2_sub2_sqr_cols; col++) {
      for (row = 0; row < public.in2_sub2_sqr_rows; row++) {
        ...
        // squaring
      private
        .d_in2_sub2_sqr[col * public.in2_sub2_sqr_rows + row] = temp2 * temp2;
    ...
    //==================================================
    //	2) GET VERTICAL CUMULATIVE SUM SAVE
    // IN
    // d_in2_pad
    //==================================================
  ...
  private
    .d_tMask[tMask_col * public.tMask_rows + tMask_row] = 1;
    ...
    pointer = private.point_no * public.frames + public.frame_no;
  private
    .d_tRowLoc[pointer] = private.d_Row[private.point_no] + offset_row;
  private
    .d_tColLoc[pointer] = private.d_Col[private.point_no] + offset_col;
  }
  ...
  private
    .d_Row[private.point_no] = private.d_tRowLoc[loc_pointer];
  private
    .d_Col[private.point_no] = private.d_tColLoc[loc_pointer];
